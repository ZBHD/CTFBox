...
class Miscellaneous(object):
    ...
    def getRemoteTempPath(self):
        if not conf.tmpPath and Backend.isDbms(DBMS.MSSQL):
            debugMsg = '识别 Microsoft SQL Server 错误日志目录 '
            debugMsg += 'sqlmap 将用来存储临时文件 '
            debugMsg += '命令的输出'
    ...
    def cleanup(self, onlyFileTbl=False, udfDict=None, web=False):
        ...
        if not onlyFileTbl:
            ...
            if conf.osPwn:
                warnMsg += '和 Metasploit 相关的临时文件 '
                warnMsg += "folder "

            warnMsg += '保存在文件系统上只能删除 '
            warnMsg += "manually"
            logger.warning(warnMsg)

    def likeOrExact(self, what):
        message = '您是否希望 sqlmap 考虑提供的 %s:\n' % what
        message += '[1] 如 %s 名称（默认）\n' % what
        message += '[2] 作为确切的 %s 名称' % what
