...
class Connect(object):
    ...
    def _getPageProxy(**kwargs):
        try:
            if (len(inspect.stack()) > sys.getrecursionlimit() // 2):   # 注：https://github.com/sqlmapproject/sqlmap/issues/4525
                warnMsg = '无法连接到目标 URL'
    ...
    def _retryProxy(**kwargs):
        ...
        if kb.testMode and kb.previousMethod == PAYLOAD.METHOD.TIME:
            # 基于时间的载荷可能会导致 Web 服务器无响应
            # 如果可注入的代码是某种类似 JOIN 的查询
            warnMsg = '很可能 Web 服务器实例尚未恢复 '
            warnMsg += '来自之前的基于时间的载荷。如果出现问题 '
            warnMsg += '仍然存在，请等待几分钟并重新运行 '
            warnMsg += '选项“--technique”中没有标志“T” '
            warnMsg += '（例如“--flush-session --technique=BEUS”）或尝试 '
            warnMsg += '降低选项“--time-sec”的值（例如“--time-sec=2”）'
            ...
            warnMsg = '如果问题仍然存在，请尝试降低 '
            warnMsg += '使用的线程数（选项“--threads”）'
            singleTimeWarnMessage(warnMsg)
    ...
    def getPage(**kwargs):
        ...
        if conf.liveCookies:
            with kb.locks.liveCookies:
                if not checkFile(conf.liveCookies, raiseOnError=False) or os.path.getsize(conf.liveCookies) == 0:
                    warnMsg = '[%s] [警告] 实时 cookies 文件“%s”为空或不存在。等待超时（%d秒）' % (time.strftime("%X"), conf.liveCookies, LIVE_COOKIES_TIMEOUT)
                    ...
                    if not valid:
                        errMsg = '从文件“%s”加载 cookie 时出现问题' % conf.liveCookies
        ...
        # 修复 urllib2 跳过所提供的其他部分时的已知问题
        # url 用空格字符分割，并在后期进行 urlencode 编码
        url = url.replace(" ", "%20")
        ...
        # 支持非拉丁语（例如西里尔语）URL，因为 urllib/urllib2 不支持
        # 默认支持那些
        url = asciifyUrl(url)
        ...
        try:
            ...
            if headers.get(HTTP_HEADER.CONTENT_TYPE) == POST_HINT_CONTENT_TYPES[POST_HINT.MULTIPART]:
                warnMsg = '“%s”请求头中缺少“边界参数”。 ' % HTTP_HEADER.CONTENT_TYPE
                warnMsg += '将尝试重建'
                singleTimeWarnMessage(warnMsg)
            ...
            for key, value in list(headers.items()):
                ...
                if isinstance(value, six.string_types):
                    ...
                    headers[getBytes(key) if six.PY2 else key] = getBytes(value.strip("\r\n"))  # 注意：Python3 has_header() 需要非字节值

            if six.PY2:
                url = getBytes(url)  # 注意：Python3 需要文本，而 Python2 在将文本与二进制 POST 混合时存在问题

            if webSocket:
                ws = websocket.WebSocket()
                ws.settimeout(WEBSOCKET_INITIAL_TIMEOUT if kb.webSocketRecvCount is None else timeout)
                ws.connect(url, header=("%s: %s" % _ for _ in headers.items() if _[0] not in ("Host",)), cookie=cookie)  # WebSocket 会自动添加 header 的 Host 字段
                ...
                if conf.http2:
                    try:
                        ...
                        raise SqlmapMissingDependence('httpx[http2] 不可用（例如“pip%s install httpx[http2]”）' % ('3' if six.PY3 else ""))

                    if LooseVersion(httpx.__version__) < LooseVersion(MIN_HTTPX_VERSION):
                        raise SqlmapMissingDependence('检测到过时版本的 httpx (%s<%s)' % (httpx.__version__, MIN_HTTPX_VERSION))
                ...
                if conn:
                    code = (code or conn.code) if conn.code == kb.originalCode else conn.code  # 不要覆盖重定向代码（用于比较目的）
            ...
            if not refreshing:
                ...
                if not refresh:
                    ...
                    if refresh:
                        debugMsg = '获取 Javascript 重定向逻辑'
                        logger.debug(debugMsg)

                if refresh:
                    if kb.alwaysRefresh is None:
                        msg = '有刷新意图 '
                        msg += '（将类似登录页面常见的响应重定向到“%s”。 ' % refresh
                        msg += '您想从现在开始应用它吗？ [是/否]'
            ...
            if conn and not conf.keepAlive:
                try:
                    ...
                    warnMsg = "连接关闭期间出现问题（'%s'）" % getSafeExString(ex)
                    logger.warning(warnMsg)

        except SqlmapConnectionException as ex:
            if conf.proxyList and not kb.threadException:
                warnMsg = "无法连接到目标 URL（'%s'）" % getSafeExString(ex)
            ...
            if ex.code not in (conf.ignoreCode or []):
                if ex.code == _http_client.UNAUTHORIZED:
                    ...
                    if raise404:
                        ...
                        debugMsg = '找不到页面（%d）' % code
                        singleTimeLogMessage(debugMsg, logging.DEBUG)
                elif ex.code == _http_client.GATEWAY_TIMEOUT:
                    if ignoreTimeout:
                        return None if not conf.ignoreTimeouts else "", None, None
                    else:
                        warnMsg = '无法连接到目标 URL (%d - %s)' % (ex.code, _http_client.responses[ex.code])
                        if threadData.retriesCount < conf.retries and not kb.threadException:
                            warnMsg += '。 sqlmap 将重试该请求'
                            logger.critical(warnMsg)
                            return Connect._retryProxy(**kwargs)
                        elif kb.testMode:
                            logger.critical(warnMsg)
                    ...
                    debugMsg = "收到 HTTP 错误代码：%d ('%s')" % (code, status)
                    logger.debug(debugMsg)
            ...
            if checking:
                ...
                warnMsg = 'NTLM 身份验证出现问题'
            elif "Invalid header name" in tbMsg:  # （例如 PostgreSQL ::文本载荷）
                return None, None, None
            elif "BadStatusLine" in tbMsg:
                warnMsg = '连接断开或未知 HTTP '
                warnMsg += '收到状态码'
                if not conf.agent and not conf.randomAgent:
                    warnMsg += '。尝试强制使用 HTTP User-Agent '
                    warnMsg += '带有选项“--user-agent”或开关“--random-agent”的请求头'
            elif "IncompleteRead" in tbMsg:
                warnMsg = '检索数据时出现不完整读取错误 '
                warnMsg += '从目标 URL'
        ...
        # Python3.11.0a7 的脏补丁（例如 https://github.com/sqlmapproject/sqlmap/issues/5091)
        if not sys.version.startswith("3.11."):
            if conf.retryOn and re.search(conf.retryOn, page, re.I):
                if threadData.retriesCount < conf.retries:
                    warnMsg = '由于不受欢迎的页面内容而强制重试请求'
                    logger.warning(warnMsg)
                    return Connect._retryProxy(**kwargs)
    ...
    def queryPage(value=None, place=None, content=False, getRatioValue=False, silent=False, method=None, timeBasedCompare=False, noteResponseTime=True, auxHeaders=None, response=False, raise404=None, removeReflection=True, disableTampering=False, ignoreSecondOrder=False):
        ...
        if conf.httpHeaders:
            ...
            if (kb.postHint or conf.skipUrlEncode) and postUrlEncode:
                postUrlEncode = False
                if not (conf.skipUrlEncode and contentType):    # 注：https://github.com/sqlmapproject/sqlmap/issues/5092
        ...
        if payload:
            ...
            if not disableTampering and kb.tamperFunctions:
                for function in kb.tamperFunctions:
                    ...
                    try:
                        ...
                        errMsg = '运行 tamper 时发生错误 '
                        errMsg += "function '%s' ('%s')" % (function.__name__, getSafeExString(ex))
                        raise SqlmapGenericException(errMsg)

                    if not isinstance(payload, six.string_types):
                        errMsg = '篡改函数“%s”返回 ' % function.__name__
                        errMsg += "载荷类型无效（'%s'）" % type(payload)
                        raise SqlmapValueException(errMsg)
            ...
            if place == PLACE.CUSTOM_POST and kb.postHint:
                if kb.postHint in (POST_HINT.SOAP, POST_HINT.XML):
                    # SOAP/XML 中的载荷应替换字符 > 和 <
                    # 及其 HTML 编码对应项
                    payload = payload.replace("&#", SAFE_HEX_MARKER)
                    payload = payload.replace('&', "&amp;").replace('>', "&gt;").replace('<', "&lt;").replace('"', "&quot;").replace("'", "&apos;")  # 参考号：https://stackoverflow.com/a/1091953
                    payload = payload.replace(SAFE_HEX_MARKER, "&#")
                ...
                # GET、POST、URI 和 Cookie 负载需要进行彻底的 URL 编码
                if (place in (PLACE.GET, PLACE.URI, PLACE.COOKIE) or place == PLACE.CUSTOM_HEADER and value.split(',')[0].upper() == HTTP_HEADER.COOKIE.upper()) and not conf.skipUrlEncode or place in (PLACE.POST, PLACE.CUSTOM_POST) and postUrlEncode:
                    ...
                    if place == PLACE.COOKIE or place == PLACE.CUSTOM_HEADER and value.split(',')[0].upper() == HTTP_HEADER.COOKIE.upper():
                        if kb.choices.cookieEncode is None:
                            msg = '您想要对 cookie 值进行 URL 编码（特定于实现）吗？ %s' % ("[Y/n]" if not conf.url.endswith(".aspx") else "[y/N]")  # 参考号：https://support.microsoft.com/en-us/kb/313282
                    ...
                    if not skip:
                        if place in (PLACE.POST, PLACE.CUSTOM_POST):  # 其他情况下的潜在问题（例如整个 URI 的 URL 编码 - 包括路径）
            ...
            if conf.hpp:
                ...
                if place in (PLACE.GET, PLACE.POST):
                    ...
                    warnMsg = 'HTTP 参数污染仅适用于常规 '
                    warnMsg += 'GET 和 POST 参数'
                    singleTimeWarnMessage(warnMsg)
        ...
        elif place == PLACE.GET:  # 注意：对于（例如）checkWaf()，当没有 GET 参数时
            get = value
        ...
        if conf.csrfToken:
            ...
            for attempt in xrange(conf.csrfRetries + 1):
                ...
                if attempt > 0:
                    warnMsg = '无法在“%s”处找到反 CSRF 令牌“%s”' % (conf.csrfToken._original, conf.csrfUrl or conf.url)
                    warnMsg += '。 sqlmap 将重试该请求'
                    logger.warning(warnMsg)
                ...
                page = urldecode(page)  # 对于名称中包含特殊字符的反 CSRF 令牌（例如“foo:bar=...”）
            ...
            if not token:
                errMsg = '在“%s”处找不到反 CSRF 令牌“%s”' % (conf.csrfToken._original, conf.csrfUrl or conf.url)
                if not conf.csrfUrl:
                    errMsg += '。您可以尝试通过提供重新运行 '
                    errMsg += '选项“--csrf-url”的有效值'
        ...
        if timeBasedCompare and not conf.disableStats:
            if len(kb.responseTimes.get(kb.responseTimeMode, [])) < MIN_TIME_RESPONSES:
                ...
                if conf.tor:
                    warnMsg = '强烈建议避免使用开关“--tor” '
                    warnMsg += '由于固有的高延迟时间而进行基于时间的注入'
                    singleTimeWarnMessage(warnMsg)

                warnMsg = '[%s] [警告] %s 基于时间的比较需要 ' % (time.strftime("%X"), "(case) " if kb.responseTimeMode else "")
                warnMsg += '%s 统计模型，请稍候' % ("larger" if len(kb.responseTimes) == 1 else "reset of")
                dataToStdout(warnMsg)
                ...
                warnMsg = '不要对网络连接造成压力，这一点非常重要 '
                warnMsg += '在使用基于时间的载荷期间，以防止潜在的 '
                warnMsg += "disruptions "
                singleTimeWarnMessage(warnMsg)
            ...
            if not kb.laggingChecked:
                ...
                if deviation is not None and deviation > WARN_TIME_STDEV:
                    ...
                    warnMsg = '已检测到相当大的滞后 '
                    warnMsg += '在连接响应中。请尽可能使用高 '
                    warnMsg += '选项“--time-sec”的值尽可能（例如 '
                    warnMsg += '10 个或更多）'
                    logger.critical(warnMsg)
        ...
        if pageLength is None:
            try:
                ...
                warnMsg = '网站返回了非常大的响应'
                if kb.testMode:
                    warnMsg += ' 处于测试阶段。这是一个常见的 '
                    warnMsg += '自定义 WAF/IPS 解决方案中的行为'
                singleTimeWarnMessage(warnMsg)
