...
class Enumeration(GenericEnumeration):
    ...
    def getPrivileges(self, *args, **kwargs):
        warnMsg = '在 Sybase 上无法获取 '
        warnMsg += '数据库用户权限，sqlmap会检查是否 '
        warnMsg += '数据库用户是否是数据库管理员'
    ...
    def getColumns(self, onlyColNames=False, colTuple=None, bruteForce=None, dumpMode=False):
        ...
        if conf.tbl:
            ...
            if len(kb.data.cachedTables) > 0:
                ...
                errMsg = '无法检索表 '
                errMsg += '在数据库“%s”上' % unsafeSQLIdentificatorNaming(conf.db)
        ...
        for tbl in tblList:
            ...
            infoMsg += '在数据库“%s”上' % unsafeSQLIdentificatorNaming(conf.db)
            logger.info(infoMsg)
    ...
    def searchDb(self):
        warnMsg = 'Sybase 数据库搜索未实现'
        logger.warning(warnMsg)

        return []

    def searchTable(self):
        warnMsg = 'Sybase 上的表搜索未实现'
        logger.warning(warnMsg)

        return []

    def searchColumn(self):
        warnMsg = 'Sybase 上的列搜索未实现'
        logger.warning(warnMsg)

        return []

    def search(self):
        warnMsg = 'Sybase 搜索选项不可用'
        logger.warning(warnMsg)

    def getHostname(self):
        warnMsg = '在 Sybase 上，无法枚举主机名'
        logger.warning(warnMsg)

    def getStatements(self):
        warnMsg = '在 Sybase 上，无法枚举 SQL 语句'
        logger.warning(warnMsg)
