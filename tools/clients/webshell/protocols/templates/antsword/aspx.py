"""蚁剑 ASPX 载荷（C# 源码字符串形式）。

蚁剑 ASPX 一句话通常形如 `<%@ Page ... %><% Response.Write(new System.Diagnostics.Process...) %>`
或 `eval(Request.Item[pass])`。本引擎沿用 `->|` / `|<-` 标记与 `/*CTFBOX_AS|...*/`
标签约定，测试解释器据此仿真载荷行为。
"""
...
def render(operation: str, params: dict, marker: tuple[str, str]) -> str:
    ...
    raise ValueError(f"蚁剑 ASPX 载荷不支持操作：{operation}")
