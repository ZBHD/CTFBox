...
# 参考号：https://mail.python.org/pipermail/python-list/2009-November/558615.html
try:
    WindowsError
except NameError:
    WindowsError = None
...
def _loadQueries():
    ...
    try:
        ...
        errMsg = '似乎有问题 '
        errMsg += '文件“%s”（“%s”）。请使 ' % (paths.QUERIES_XML, getSafeExString(ex))
...
def _setMultipleTargets():
    ...
    debugMsg = '解析“%s”中的目标列表' % conf.logFile
    ...
    if updatedTargetsCount > initialTargetsCount:
        infoMsg = 'sqlmap解析%d ' % (updatedTargetsCount - initialTargetsCount)
        infoMsg += '（参数唯一）来自的请求 '
        infoMsg += '准备测试的目标列表'
        logger.info(infoMsg)
...
def _setRequestFromFile():
    ...
    if conf.requestFile:
        for requestFile in re.split(PARAMETER_SPLITTING_REGEX, conf.requestFile):
            ...
            if url is None:
                errMsg = "指定文件'%s' " % requestFile
                errMsg += '不包含可用的 HTTP 请求（带参数）'
    ...
    if conf.secondReq:
        ...
        if not checkFile(conf.secondReq, False):
            errMsg = "指定二阶HTTP请求文件'%s' " % conf.secondReq
            errMsg += '不存在'
            raise SqlmapFilePathException(errMsg)

        infoMsg = '解析来自“%s”的二阶 HTTP 请求' % conf.secondReq
        logger.info(infoMsg)
...
def _doSearch():
        ...
        for link in links:
            ...
            if re.search(r"(.*?)\?(.+)", link) or conf.forms:
                ...
                if kb.data.onlyGETs is None and conf.data is None and not conf.googleDork:
                    message = '您想只扫描包含 GET 参数的结果吗？ [是/否] '
    ...
    while True:
        ...
        if kb.targets:
            infoMsg = '找到了适合您的 %d 结果 ' % len(links)
            infoMsg += '搜索呆子表情'
            ...
            message += '为你的搜索呆子表达，但他们都没有 '
            message += '有 GET 参数来测试 SQL 注入。 '
            message += '您想跳到下一个结果页面吗？ [是/否]'
...
def _setBulkMultipleTargets():
    ...
    infoMsg = '解析“%s”中的多个目标列表' % conf.bulkFile
    logger.info(infoMsg)
        ...
        for i in xrange(len(targets)):
            try:
                ...
                if conf.verbose in (1, 2):
                    status = '%d/%d 链接已访问 (%d%%)' % (i + 1, len(targets), round(100.0 * (i + 1) / len(targets)))
                    dataToStdout("\r[%s] [INFO] %s" % (time.strftime("%X"), status), True)
            except KeyboardInterrupt:
                break
            except Exception as ex:
                errMsg = '在“%s”（“%s”）搜索表单时出现问题' % (target, getSafeExString(ex))
                logger.error(errMsg)
...
def _setMetasploit():
    ...
    if IS_WIN:
        try:
            ...
            errMsg += '为了使用 Metasploit 功能 '
            errMsg += '视窗。您可以从以下位置下载： '
    ...
    if conf.osSmb:
        ...
        if not isAdmin:
            ...
            errMsg += '如果你想执行 SMB 中继攻击，因为 '
            errMsg += '它需要监听用户指定的 SMB '
            errMsg += '用于传入连接尝试的 TCP 端口'
    ...
    if conf.msfPath:
        ...
        if msfEnvPathExists:
            debugMsg = '提供的 Metasploit 框架路径 '
            debugMsg += '“%s”有效' % conf.msfPath
            logger.debug(debugMsg)
        else:
            warnMsg = '提供的 Metasploit 框架路径 '
            warnMsg += '“%s”无效。原因可能是 ' % conf.msfPath
            warnMsg += '该路径不存在或该路径不存在 '
            warnMsg += '或更多所需的 Metasploit 可执行文件 '
            warnMsg += '在 msfcli、msfconsole、msfencode 和 '
            warnMsg += 'msfpayload不存在'
            logger.warning(warnMsg)
    else:
        warnMsg = '您没有提供 Metasploit 所在的本地路径 '
        warnMsg += '框架已安装'
        logger.warning(warnMsg)

    if not msfEnvPathExists:
        warnMsg = 'sqlmap 将寻找 Metasploit 框架 '
        warnMsg += '安装在环境路径内'
        logger.warning(warnMsg)
        ...
        for envPath in envPaths:
            ...
            if any(os.path.exists(normalizePath(os.path.join(envPath, "%s%s" % (_, ".bat" if IS_WIN else "")))) for _ in ("msfcli", "msfconsole")):
                ...
                if msfEnvPathExists:
                    infoMsg = 'Metasploit框架已找到 '
                    infoMsg += '安装在“%s”路径中' % envPath
                    logger.info(infoMsg)
    ...
    if not msfEnvPathExists:
        errMsg = '无法找到 Metasploit 框架安装。 '
        errMsg += '您可以在“https://www.metasploit.com/download/”处获取'
        raise SqlmapFilePathException(errMsg)
...
def _setWriteFile():
    ...
    if not conf.fileDest:
        errMsg = '您没有提供后端数据库管理系统 绝对路径 '
        errMsg += '您要在其中写入本地文件“%s”' % conf.fileWrite
...
def _setOS():
    ...
    if conf.os.lower() not in SUPPORTED_OS:
        errMsg = '您提供了不受支持的后端数据库管理系统 操作 '
        errMsg += '系统。 OS 支持的 DBMS 操作系统 '
        errMsg += '文件系统访问权限为%s。 ' % ', '.join([o.capitalize() for o in SUPPORTED_OS])
        errMsg += '如果您不了解后端数据库管理系统底层操作系统， '
        errMsg += '不提供它，sqlmap 会对其进行指纹识别 '
        errMsg += "you."
        raise SqlmapUnsupportedDBMSException(errMsg)

    debugMsg = '强制后端数据库管理系统 操作系统为用户定义 '
    ...
    if conf.technique and isinstance(conf.technique, six.string_types):
        ...
        for letter in conf.technique.upper():
            if letter not in validLetters:
                errMsg = '--technique 的值必须是组成的字符串 '
                errMsg += '由字母 %s 组成。请参阅 ' % ", ".join(validLetters)
                errMsg += '用户手册了解详细信息'
                raise SqlmapSyntaxException(errMsg)
...
def _setDBMS():
    ...
    if conf.dbms not in SUPPORTED_DBMS:
        ...
        errMsg += '系统。支持的 DBMS 如下：%s。 ' % ', '.join(sorted((_ for _ in (list(DBMS_DICT) + getPublicTypeMembers(FORK, True))), key=str.lower))
        errMsg += '如果您不了解后端数据库管理系统，请勿提供 '
        errMsg += '它和 sqlmap 将为您识别它的指纹。'
        raise SqlmapUnsupportedDBMSException(errMsg)
        ...
        for script in re.split(PARAMETER_SPLITTING_REGEX, conf.tamper):
            ...
            try:
                if not script:
                    ...
                    errMsg = '篡改脚本“%s”应具有扩展名“.py”' % script
                    raise SqlmapSyntaxException(errMsg)
            except UnicodeDecodeError:
                errMsg = '选项“--tamper”中提供的字符无效'
                raise SqlmapSyntaxException(errMsg)

            dirname, filename = os.path.split(script)
            dirname = os.path.abspath(dirname)

            infoMsg = '正在加载篡改模块“%s”' % filename[:-3]
            logger.info(infoMsg)

            if not os.path.exists(os.path.join(dirname, "__init__.py")):
                errMsg = '确保有一个空文件“__init__.py” '
                errMsg += '篡改脚本目录“%s”内部' % dirname
                raise SqlmapGenericException(errMsg)
            ...
            try:
                ...
                raise SqlmapSyntaxException('无法导入篡改模块“%s”(%s)' % (getUnicode(filename[:-3]), getSafeExString(ex)))
            ...
            for name, function in inspect.getmembers(module, inspect.isfunction):
                if name == "tamper" and (hasattr(inspect, "signature") and all(_ in inspect.signature(function).parameters for _ in ("payload", "kwargs")) or inspect.getargspec(function).args and inspect.getargspec(function).keywords == "kwargs"):
                    ...
                    try:
                        ...
                        errMsg = '检查依赖关系时发生错误 '
                        errMsg += '用于防拆模块“%s”（“%s”）' % (getUnicode(filename[:-3]), getSafeExString(ex))
                        raise SqlmapGenericException(errMsg)

            if not found:
                errMsg = '缺少函数“tamper(payload, **kwargs)” '
                errMsg += '在篡改脚本“%s”中' % script
                raise SqlmapGenericException(errMsg)

        if kb.tamperFunctions and len(kb.tamperFunctions) > 3:
            warnMsg = '使用太多的篡改脚本通常不会 '
            warnMsg += '好主意'
            logger.warning(warnMsg)
            ...
            for name, function in inspect.getmembers(module, inspect.isfunction):
                try:
                    if name == "preprocess" and inspect.getargspec(function).args and all(_ in inspect.getargspec(function).args for _ in ("req",)):
                        ...
                        break
                except ValueError:  # 注：https://github.com/sqlmapproject/sqlmap/issues/4357
                    pass

            if not found:
                errMsg = '缺少函数“预处理(req)” '
                errMsg += '在预处理脚本“%s”中' % script
                raise SqlmapGenericException(errMsg)
            else:
                try:
                    ...
                    errMsg = '函数“预处理（请求）” '
                    errMsg += '在预处理脚本“%s”中 ' % script
                    errMsg += '在测试运行中遇到问题（“%s”）。 ' % getSafeExString(ex)
        ...
        for script in re.split(PARAMETER_SPLITTING_REGEX, conf.postprocess):
            ...
            if not found:
                ...
                try:
                    ...
                    errMsg = '函数“后处理（页面，标题=无，代码=无）” '
                    errMsg += '在后处理脚本“%s”中 ' % script
                    errMsg += '应该返回一个元组“（页面，标题，代码）” '
                    errMsg += '（注意：在“%s”处找到模板脚本）' % filename
                    raise SqlmapGenericException(errMsg)
    ...
    with kb.locks.handlers:
        ...
        if conf.proxy:
            ...
            try:
                ...
                errMsg = '无效代理地址“%s”（“%s”）' % (conf.proxy, getSafeExString(ex))
                raise SqlmapSyntaxException(errMsg)
            ...
            if not all((scheme, hasattr(PROXY_TYPE, scheme), hostname, port)):
                errMsg = '代理值的格式必须为“(%s)://地址:端口”' % "|".join(_[0].lower() for _ in getPublicTypeMembers(PROXY_TYPE))
                raise SqlmapSyntaxException(errMsg)
            ...
            if scheme in (PROXY_TYPE.SOCKS4, PROXY_TYPE.SOCKS5):
                ...
                if conf.proxyCred:
                    # 参考号：http://stackoverflow.com/questions/34079/how-to-specify-an-authenticated-proxy-for-a-python-http-connection
        ...
        # 参考号：http://www.w3.org/Protocols/rfc2616/rfc2616-sec8.html
        if conf.keepAlive:
            warnMsg = '持久 HTTP(s) 连接，Keep-Alive，具有 '
            warnMsg += '由于不兼容而被禁用 '

            if conf.proxy:
                warnMsg += '使用 HTTP(s) 代理'
                logger.warning(warnMsg)
            elif conf.authType:
                warnMsg += '具有身份验证方法'
                logger.warning(warnMsg)
        ...
        opener.addheaders = []  # 注意：清除默认的“User-Agent: Python-urllib/X.Y”
...
def _setHTTPAuthentication():
    ...
    if conf.authFile and not conf.authType:
        ...
        errMsg = '您指定了 HTTP 身份验证类型，但是 '
        errMsg += '没有提供凭据'
        raise SqlmapSyntaxException(errMsg)

    elif not conf.authType and conf.authCred:
        errMsg = '您指定了 HTTP 身份验证凭据， '
        errMsg += '但没有提供类型（例如 --auth-type="basic"）'
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.headers:
        ...
        for headerValue in conf.headers:
            ...
            if headerValue.count(':') >= 1:
                ...
                errMsg = '无效请求头值：%s。有效请求头格式为“名称：值”' % repr(headerValue).lstrip('u')
                raise SqlmapSyntaxException(errMsg)
        ...
        # 使两者之间的任何缓存机制失效
        # 参考号：http://stackoverflow.com/a/1383359
...
def _setHTTPUserAgent():
    ...
    debugMsg = '设置 HTTP User-Agent 请求头'
    logger.debug(debugMsg)
    ...
    if conf.mobile:
        if conf.randomAgent:
            ...
            message = '您希望 sqlmap 模仿哪款智能手机 '
            message += '通过 HTTP User-Agent 请求头？\n'
        ...
        infoMsg = '从以下位置获取随机 HTTP User-Agent 请求头值“%s” ' % userAgent
...
def _setHostname():
    ...
    if conf.url:
        try:
            ...
            errMsg = '问题发生时 '
            errMsg += "解析 URL '%s' ('%s')" % (conf.url, getSafeExString(ex))
            raise SqlmapDataException(errMsg)
...
def _setHTTPTimeout():
    ...
    if conf.timeout:
        ...
        if conf.timeout < 3.0:
            warnMsg = '最小 HTTP 超时为 3 秒，sqlmap '
            warnMsg += '将重置它'
            logger.warning(warnMsg)
...
def _createHomeDirectories():
    ...
    for context in ("output", "history"):
        directory = paths["SQLMAP_%s_PATH" % getUnicode(context).upper()]   # 注：https://github.com/sqlmapproject/sqlmap/issues/4363
        try:
            ...
            warnMsg = '无法到达 %s %s 目录 ' % ("create" if not os.path.isdir(directory) else "write to the", context)
...
def _pympTempLeakPatch(tempDir):  # 交叉引用功能
...
def _createTemporaryDirectory():
    ...
    if conf.tmpDir:
        ...
        try:
            ...
            warnMsg = '访问时出现问题 '
            warnMsg += "系统的临时目录位置（'%s'）。请 " % getSafeExString(ex)
            warnMsg += '确保有足够的剩余磁盘空间。如果问题仍然存在， '
...
def _cleanupOptions():
    ...
    if conf.csrfToken:
        ...
        try:
            ...
            if re.escape(conf.csrfToken) != conf.csrfToken:
                message = '为选项“--csrf-token”提供的值是正则表达式吗？ [是/否] '
    ...
    if "timeSec" not in kb.explicitSettings:
        if conf.tor:
            ...
            warnMsg = '增加默认值 '
            warnMsg += '选项“--time-sec”为 %d，因为 ' % conf.timeSec
            warnMsg += '提供了开关“--tor”'
            logger.warning(warnMsg)
    else:
        kb.adjustTimeDelay = ADJUST_TIME_DELAY.DISABLE
...
def _useWizardInterface():
    ...
    while not conf.url:
        message = '请输入完整的目标网址（-u）： '
        conf.url = readInput(message, default=None, checkBatch=False)

    message = '%s 数据 (--data) [无输入]： ' % ((conf.method if conf.method != HTTPMETHOD.GET else None) or HTTPMETHOD.POST)
    ...
    if not (any('=' in _ for _ in (conf.url, conf.data)) or '*' in conf.url):
        ...
        warnMsg += '（例如“http://www.site.com/vuln.php?id=1”中的 GET 参数“id”）。 '
    ...
    if not conf.getAll:
        ...
        while choice is None or choice not in ("", "1", "2", "3"):
            message = '枚举（--banner/--current-user/等）。请选择：\n'
            message += '[1] 基础（默认）\n[2] 中级\n[3] 全部'
            choice = readInput(message, default='1')
    ...
    logger.debug('静音 sqlmap..它会给你带来魔力')
    conf.verbose = 0
...
def _saveConfig():
    ...
    debugMsg = '将命令行选项保存到 sqlmap 配置 INI 文件'
    logger.debug(debugMsg)
...
def _setDNSServer():
    ...
    if isAdmin:
        try:
            ...
            errMsg = '设置时出错 '
            errMsg += "DNS 服务器实例（'%s'）" % getSafeExString(ex)
            raise SqlmapGenericException(errMsg)
    else:
        errMsg = '您需要以管理员身份运行 sqlmap '
        errMsg += '如果您想执行 DNS 数据泄露攻击 '
        errMsg += '因为它需要监听特权 UDP 端口 53 '
        errMsg += '用于传入地址解析尝试'
        raise SqlmapMissingPrivileges(errMsg)
...
def _setTorHttpProxySettings():
    ...
    if port:
        ...
        errMsg = '无法与 Tor HTTP 代理建立连接。 '
        errMsg += '请确保您已安装并设置 Tor（捆绑包） '
        errMsg += '这样您就可以成功使用开关“--tor” '
...
def _setTorSocksProxySettings():
    ...
    if not port:
        errMsg = '无法与 Tor SOCKS 代理建立连接。 '
        errMsg += '请确保您已安装并设置 Tor 服务 '
        errMsg += '这样您就可以成功使用开关“--tor” '
        raise SqlmapConnectionException(errMsg)

    # SOCKS5防止DNS泄漏（http://en.wikipedia.org/wiki/Tor_%28anonymity_network%29)
...
def _checkTor():
    ...
    if not tor_status or not tor_status.get("IsTor"):
        errMsg = '看来 Tor 设置不正确。请尝试使用选项“--tor-type”和/或“--tor-port”'
        raise SqlmapConnectionException(errMsg)
    else:
        infoMsg = 'Tor 正在正确使用'
        logger.info(infoMsg)
...
def _basicOptionValidation():
    ...
    if isinstance(conf.limitStart, int) and conf.limitStart > 0 and \
       isinstance(conf.limitStop, int) and conf.limitStop < conf.limitStart:
        warnMsg = '使用大于 --stop (limitStop) 选项值的选项“--start”(limitStart) 被认为是不稳定的'
        logger.warning(warnMsg)
    ...
    if conf.proxyFile and not any((conf.randomAgent, conf.mobile, conf.agent, conf.requestFile)):
        warnMsg = '强烈建议在以下情况下使用开关“--random-agent” '
        warnMsg += '使用选项“--proxy-file”'
        logger.warning(warnMsg)
    ...
    if conf.crawlDepth:
        try:
            ...
            errMsg = '用于选项“--crawl”的值无效（“%s”）' % getSafeExString(ex)
            raise SqlmapSyntaxException(errMsg)
    ...
    if conf.retryOn:
        ...
        if conf.retries == defaults.retries:
            ...
            warnMsg = '增加默认值 '
            warnMsg += '选项“--retries”为 %d，因为 ' % conf.retries
    ...
    if conf.threads > MAX_NUMBER_OF_THREADS and not conf.get("skipThreadCheck"):
        errMsg = '使用的最大线程数为 %d，避免潜在的连接问题' % MAX_NUMBER_OF_THREADS
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.checkTor and not any((conf.tor, conf.proxy)):
        errMsg = '开关“--check-tor”需要使用开关“--tor”（或选项“--proxy”以及 Tor 服务的 HTTP 代理地址）'
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.uValues and (not re.search(r"\A['\w\s.,()%s-]+\Z" % CUSTOM_INJECTION_MARK_CHAR, conf.uValues) or conf.uValues.count(CUSTOM_INJECTION_MARK_CHAR) != 1):
        errMsg = '选项“--union-values”必须包含有效的 UNION 列值以及注入位置 '
    ...
    if conf.dbmsCred and ':' not in conf.dbmsCred:
        errMsg = '选项“--dbms-cred”的值必须位于 '
        errMsg += '格式<用户名>:<密码>（例如“root:pass”）'
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.encoding:
        ...
        if _ is None:
            ...
            errMsg += "'%s' 获取完整列表 " % CODECS_LIST_PAGE
            errMsg += '支持的编码'
            raise SqlmapSyntaxException(errMsg)
        else:
            conf.encoding = _
    ...
    if conf.loadCookies and not os.path.exists(conf.loadCookies):
        errMsg = 'cookie 文件“%s”不存在' % os.path.abspath(conf.loadCookies)
        raise SqlmapFilePathException(errMsg)
