...
def _search(dork):
    ...
    logger.info('使用搜索结果页面#%d' % gpage)

    url = "https://www.google.com/search?"                                  # 注意：如果同意失败，请尝试使用“http://”
    ...
    if not retVal:
        message = '没有找到可用的链接。你想让我做什么？'
        message += '\n[1]（重新）尝试使用 DuckDuckGo（默认）'
        message += '\n[2]（重新）尝试使用 Bing'
        message += "\n[3] quit"
        ...
        if not retVal and "issue with the Tor Exit Node you are currently using" in page:
            warnMsg = 'DuckDuckGo 检测到“异常”流量 '
            warnMsg += '使用的 (Tor) IP 地址'
