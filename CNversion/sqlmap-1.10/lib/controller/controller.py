...
def start():
    ...
    for targetUrl, targetMethod, targetData, targetCookie, targetHeaders in kb.targets:
        ...
        try:
            ...
            if conf.data:
                # 注意：显式 URL 编码 __ ASP(.NET) 参数（例如，避免 Base64 编码的“+”字符出现问题）- Web 浏览器中的标准过程
            ...
            if testSqlInj and conf.hostname in kb.vulnHosts:
                if kb.skipVulnHost is None:
                    message = 'SQL注入漏洞已经被检测到 '
                    message += '针对“%s”。你想跳过吗 ' % conf.hostname
                    message += '涉及它的进一步测试？ [是/否]'
                ...
                if conf.forms and conf.method:
                    ...
                    if choice == 'N':
                        ...
                        if conf.method != HTTPMETHOD.GET:
                            message = '编辑%s数据[默认：%s]%s： ' % (conf.method, urlencode(conf.data or "") if re.search(r"\A\s*[<{]", conf.data or "None") is None else conf.data, " (Warning: blank fields detected)" if conf.data and extractRegexResult(EMPTY_FORM_FIELDS_REGEX, conf.data) else "")
                            ...
                            if '?' in targetUrl:
                                ...
                                message = '编辑GET数据[默认：%s]： ' % secondPart
            ...
            if (len(kb.injections) == 0 or (len(kb.injections) == 1 and kb.injections[0].place is None)) and (kb.injection.place is None or kb.injection.parameter is None):
                ...
                for place in parameters:
                    # 仅在以下情况下测试 User-Agent 和 Referer 请求头
                    # --level >= 3
                    ...
                    for parameter, value in paramDict.items():
                        ...
                        if paramKey in kb.testedParams:
                            ...
                            infoMsg = "跳过先前处理的 %s 参数 '%s'" % ("%s " % paramType if paramType != parameter else "", parameter)
                            logger.info(infoMsg)
                            ...
                            infoMsg = '跳过随机 %s 参数“%s”' % ("%s " % paramType if paramType != parameter else "", parameter)
                            logger.info(infoMsg)
                        ...
                        # 忽略 --level < 4 的类似会话参数
                            ...
                            if not check:
                                ...
                                infoMsg = '%s 参数“%s”似乎是动态的' % ("%s " % paramType if paramType != parameter else "", parameter)
                                logger.info(infoMsg)
                        ...
                        if testSqlInj:
                            try:
                                ...
                                infoMsg = '测试 %s 参数“%s”上的 SQL 注入' % ("%s " % paramType if paramType != parameter else "", parameter)
                                logger.info(infoMsg)
                                ...
                                if getattr(injection, "place", None) is not None:
                                    if NOTE.FALSE_POSITIVE_OR_UNEXPLOITABLE in injection.notes:
                                        ...
                                        if not kb.alerted:
                                            if conf.alert:
                                                infoMsg = "执行警报 shell 命令（'%s'）" % conf.alert
                                                logger.info(infoMsg)
                                                try:
                                                    process = subprocess.Popen(conf.alert, shell=True)
                                                    process.wait()
                                                except Exception as ex:
                                                    errMsg = '执行“%s”（“%s”）时发生错误' % (conf.alert, getSafeExString(ex))
                                                    logger.error(errMsg)
                                        ...
                                        msg += '是脆弱的。您想继续测试其他人（如果有的话）吗？ [是/否] '
            ...
            if len(kb.injections) == 0 or (len(kb.injections) == 1 and kb.injections[0].place is None):
                if kb.vainRun and not conf.multipleTargets:
                    ...
                    errMsg += '（例如，在“www.site.com/index.php?id=1”中获取参数“id”）'
                    if kb.originalPage:
                        advice = []
                        ...
                        if advice:
                            errMsg += '。建议您使用“%s”重新运行' % ' '.join(advice)
                    raise SqlmapNoneDataException(errMsg)
                else:
                    errMsg = '所有测试的参数似乎都不可注射。'
                    ...
                    if isinstance(conf.technique, list) and len(conf.technique) < 5:
                        errMsg += ' 重新运行而不提供选项“--technique”。'

                    if not conf.textOnly and kb.originalPage:
                        percent = (100.0 * len(getFilteredPageContent(kb.originalPage)) / len(kb.originalPage))

                        if kb.dynamicMarkings:
                            errMsg += ' 您可以使用开关“--text-only”来尝试一下 '
                            errMsg += '如果目标页面的百分比较低 '
                            errMsg += '文本内容 (~%.2f%% ' % percent
                            errMsg += '页面内容为文本）。'
                        elif percent < LOW_TEXT_PERCENT and not kb.errorIsNone:
                            errMsg += ' 请使用开关“--text-only”重试 '
                            errMsg += '（与 --technique=BU 一起）如本例 '
                            errMsg += '看起来是一个完美的候选人 '
                            errMsg += '（文本内容低且无法 '
                            errMsg += '比较引擎至少检测 '
                            errMsg += '一个动态参数）。'
                    ...
                    if kb.heuristicTest == HEURISTIC_TEST.POSITIVE:
                        errMsg += ' 由于启发式测试结果呈阳性，您是 '
                        errMsg += '强烈建议继续测试。'

                    if conf.string:
                        errMsg += ' 另外，您可以尝试通过提供 '
                        errMsg += '选项“--string”的有效值可能是您的字符串 '
                        errMsg += '所选择的不匹配 '
                        errMsg += '完全真实的反应。'
                    elif conf.regexp:
                        errMsg += ' 另外，您可以尝试通过提供 '
                        errMsg += '选项“--regexp”的有效值可能与常规值一样 '
                        errMsg += '您选择的表达方式 '
                        errMsg += '不完全匹配 True 响应。'

                    if not conf.tamper:
                        errMsg += ' 如果您怀疑存在某种保护机制 '
                        errMsg += '涉及（例如WAF）也许你可以尝试使用 '
                        errMsg += '选项“--tamper”（例如“--tamper=space2comment”）'

                        if not conf.randomAgent:
                            errMsg += ' 和/或开关“--random-agent”'
            ...
            if kb.lastCtrlCTime and (time.time() - kb.lastCtrlCTime < 1):
                ...
                raise SqlmapUserQuitException('用户中止（多次按下 Ctrl+C）')
    ...
    if kb.dataOutputFlag and not conf.multipleTargets:
        logger.info('获取的数据记录到“%s”下的文本文件中' % conf.outputPath)

    if conf.multipleTargets:
        if conf.resultsFile:
            infoMsg = '您可以找到多个目标的扫描结果 '
            infoMsg += 'CSV 文件“%s”内的模式' % conf.resultsFile
            logger.info(infoMsg)
