...
class Filesystem(object):
        ...
        if fileRead and Backend.isDbms(DBMS.PGSQL):
            ...
            if isNumPosStrValue(remoteFileSize):
                ...
                if localFileSize == remoteFileSize:
                    ...
                    infoMsg += "'%s' 具有相同的尺寸 (%d B)" % (remoteFile, localFileSize)
                ...
                sameFile = False
                warnMsg = '看起来文件还没有被写入（通常 '
                warnMsg += '如果 DBMS 进程用户没有写入，则会发生 '
                warnMsg += '目标路径中的权限）'
                logger.warning(warnMsg)
    ...
    def askCheckWrittenFile(self, localFile, remoteFile, forceCheck=False):
        ...
        if forceCheck is not True:
            message = '您是否想要确认本地文件“%s” ' % localFile
            message += '已成功写入后端数据库管理系统 '
            message += "文件系统（'%s'）？ [是/否] " % remoteFile
    ...
    def askCheckReadFile(self, localFile, remoteFile):
        if not kb.bruteMode:
            message = '您是否想要确认远程文件“%s” ' % remoteFile
            message += '已从后端下载成功 '
            message += 'DBMS 文件系统？ [是/否] '
        ...
        for remoteFile in remoteFile.split(','):
            ...
            if conf.direct or isStackingAvailable():
                if isStackingAvailable():
                    debugMsg = '将尝试使用堆叠查询 SQL 读取文件 '
                    debugMsg += '注射技术'
                    logger.debug(debugMsg)

                fileContent = self.stackedReadFile(remoteFile)
            elif Backend.isDbms(DBMS.MYSQL):
                debugMsg = '将尝试使用非堆叠查询读取文件 '
                debugMsg += 'SQL注入技术'
                logger.debug(debugMsg)

                fileContent = self.nonStackedReadFile(remoteFile)
            else:
                errMsg = '检测到的任何 SQL 注入技术都不能 '
                errMsg += '用于从底层文件读取文件 '
                errMsg += '后端%s服务器系统' % Backend.getDbms()
                logger.error(errMsg)
