...
def _findUnionCharCount(comment, place, parameter, value, prefix, suffix, where=PAYLOAD.WHERE.ORIGINAL):
    ...
    def _orderByTechnique(lowerCount=None, upperCount=None):
        ...
        if _orderByTest(1 if lowerCount is None else lowerCount) and not _orderByTest(randomInt() if upperCount is None else upperCount + 1):
            infoMsg = '“ORDER BY”技术似乎是可用的。 '
            infoMsg += '这应该会减少所需的时间 '
            infoMsg += '找到正确的号码 '
            infoMsg += '查询列数。自动延长 '
            infoMsg += '当前 UNION 查询注入技术测试的范围'
    ...
    try:
        ...
        if kb.orderByColumns is None and (lowerCount == 1 or conf.uCols):  # 注意：ORDER BY 并不是万无一失的
            found = _orderByTechnique(lowerCount, upperCount) if conf.uCols else _orderByTechnique()

            if found:
                kb.orderByColumns = found
                infoMsg = '目标 URL 的查询中似乎包含 %d 列 %s' % (found, 's' if found > 1 else "")
                singleTimeLogMessage(infoMsg)
                return found
            elif kb.futileUnion:
                return None
    ...
    if retVal:
        infoMsg = '目标 URL 似乎可以与 %d 列进行 UNION 注入' % retVal
...
def _unionPosition(comment, place, parameter, prefix, suffix, count, where=PAYLOAD.WHERE.ORIGINAL):
    ...
    for charCount in (UNION_MIN_RESPONSE_CHARS << 2, UNION_MIN_RESPONSE_CHARS):
        ...
        # 对于表的每一列（NULL 值）执行请求
        # 用于测试它的 UNION ALL SELECT 语句目标 URL 是
        # 受可利用的联合 SQL 注入漏洞影响
        for position in positions:
            ...
            if content and phrase in content:
                ...
                if unionErrorCase and count > 1:
                    warnMsg = '发现组合 UNION/基于错误的 SQL 注入案例 '
                    warnMsg += '列 %d。 sqlmap将尝试寻找另一个 ' % (position + 1)
                    warnMsg += '具有更好特性的色谱柱'
                    logger.warning(warnMsg)
                else:
                    break
...
def _unionTestByCharBruteforce(comment, place, parameter, value, prefix, suffix):
    ...
    if count:
        ...
        if not all((validPayload, vector)) and not all((conf.uChar, conf.dbms, kb.unionTemplate)):
            ...
            warnMsg = '如果未检测到基于 UNION 的 SQL 注入， '
            warnMsg += '请考虑 '

            if not conf.uChar and count > 1 and kb.uChar == NULL and conf.uValues is None:
                message = '无法利用 NULL 值进行注入。您想尝试为选项“--union-char”使用随机整数值吗？ [是/否] '

                if not readInput(message, default='Y', boolean=True):
                    warnMsg += '选项“--union-char”的使用 '
                    warnMsg += '（例如“--union-char=1”） '
                else:
            ...
            if not conf.dbms:
                if not conf.uChar:
                    ...
                    warnMsg += '强迫 '
                warnMsg += '后端数据库管理系统（例如“--dbms=mysql”） '

            if not all((validPayload, vector)) and not warnMsg.endswith("consider "):
                singleTimeWarnMessage(warnMsg)

        if orderBy is None and kb.orderByColumns is not None and not all((validPayload, vector)):  # 丢弃 ORDER BY 结果（不可用 - 例如可能完全无效）
