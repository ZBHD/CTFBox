    ...
    class Table(object):
        ...
        def __init__(self, parent, name, columns=None, create=True, typeless=False):
            ...
            if create:
                try:
                    ...
                    errMsg = "初始化 sqlite 数据库时出现问题（'%s'） " % getSafeExString(ex, UNICODE_ENCODING)
        ...
        def execute(self, sql, parameters=None):
            try:
                ...
                errMsg = "访问 sqlite 数据库时出现问题（'%s'） " % getSafeExString(ex, UNICODE_ENCODING)
                errMsg += '位于“%s”。请确保 ' % self.parent.dbpath
                errMsg += '它没有被其他程序使用'
                raise SqlmapGenericException(errMsg)
