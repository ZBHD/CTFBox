...
class Dump(object):
    ...
    def dbTableValues(self, tableValues):
        ...
        if count > TRIM_STDOUT_DUMP_SIZE:
            warnMsg = '控制台输出将被修剪为 '
            warnMsg += '最后 %d 行由于 ' % TRIM_STDOUT_DUMP_SIZE
            warnMsg += '大桌子尺寸'
            logger.warning(warnMsg)
                    ...
                    if len(value) > MIN_BINARY_DISK_DUMP_SIZE and r'\x' in value:
                        try:
                            ...
                            if any(mimetype.startswith(_) for _ in ("application", "image")):
                                ...
                                warnMsg = "将二进制（'%s'）内容写入文件'%s' " % (mimetype, filepath)
                                logger.warning(warnMsg)
        ...
        if conf.dumpFormat == DUMP_FORMAT.SQLITE:
            ...
            logger.info("表'%s.%s'转储到SQLITE数据库'%s'" % (db, table, replication.dbpath))
            ...
            msg = "表'%s.%s'转储到%s文件'%s'" % (db, table, conf.dumpFormat, dumpFileName)
            if not warnFile:
                logger.info(msg)
            else:
                logger.warning(msg)
