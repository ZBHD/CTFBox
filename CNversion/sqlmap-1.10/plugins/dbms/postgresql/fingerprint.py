...
class Fingerprint(GenericFingerprint):
    ...
    def getFingerprint(self):
        ...
        if fork is None:
            ...
            elif inject.checkBooleanExpression("VERSION() LIKE '%Redshift%'"):      # 参考号：https://dataedo.com/kb/query/amazon-redshift/check-server-version
                fork = FORK.REDSHIFT
            elif inject.checkBooleanExpression("VERSION() LIKE '%Greenplum%'"):     # 参考号：http://www.sqldbpros.com/wordpress/wp-content/uploads/2014/08/what-version-of-greenplum.png
                fork = FORK.GREENPLUM
            elif inject.checkBooleanExpression("VERSION() LIKE '%Yellowbrick%'"):   # 参考号：https://www.yellowbrick.com/docs/3.3/ybd_sqlref/version.html
                fork = FORK.YELLOWBRICK
            elif inject.checkBooleanExpression("VERSION() LIKE '%EnterpriseDB%'"):  # 参考号：https://www.enterprisedb.com/edb-docs/d/edb-postgres-advanced-server/user-guides/user-guide/11/EDB_Postgres_Advanced_Server_Guide.1.087.html
                fork = FORK.ENTERPRISEDB
            elif inject.checkBooleanExpression("VERSION() LIKE '%YB-%'"):           # 参考号：https://github.com/yugabyte/yugabyte-db/issues/2447#issue-499562926
                fork = FORK.YUGABYTEDB
            elif inject.checkBooleanExpression("VERSION() LIKE '%openGauss%'"):
                fork = FORK.OPENGAUSS
            elif inject.checkBooleanExpression("AURORA_VERSION() LIKE '%'"):        # 参考号：https://aws.amazon.com/premiumsupport/knowledge-center/aurora-version-number/
                fork = FORK.AURORA
            else:
                fork = ""
    ...
    def checkDbms(self):
        ...
        * https://www.postgresql.org/docs/current/static/release.html
        ...
        # 注意：Vertica 在没有 CONVERT_TO() 的情况下也可以工作
        ...
        if result:
            ...
            if not result:
                warnMsg = '后端数据库管理系统 不是 %s' % DBMS.PGSQL
    ...
    def checkDbmsOs(self, detailed=False):
        ...
        infoMsg = '后端数据库管理系统操作系统是%s' % Backend.getOs()
        logger.info(infoMsg)
