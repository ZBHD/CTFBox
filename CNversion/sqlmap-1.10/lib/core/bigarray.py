...
try:
    # Python 2：basestring 涵盖 str 和 unicode
    STRING_TYPES = (basestring,)
except NameError:
    # Python 3：str 和 bytes 是分开的
...
class BigArray(list):
    ...
    def _dump(self, chunk):
        try:
            ...
            errMsg = '存储数据时发生异常 '
            errMsg += "到临时文件（'%s'）。请 " % ex
            errMsg += '确保有足够的剩余磁盘空间。如果问题仍然存在， '
            errMsg += '尝试将环境变量“TEMP”设置为某个位置 '
            errMsg += '当前用户可写'
            raise SqlmapSystemException(errMsg)
    ...
    def __getitem__(self, y):
        with self._lock:
            length = len(self)
            if length == 0:
                raise IndexError('BigArray 索引超出范围')
