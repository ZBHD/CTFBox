...
class Takeover(GenericTakeover):
    ...
    def udfSetLocalPaths(self):
        ...
        if not banVer or not banVer[0].isdigit():
            errMsg = '未知版本的 PostgreSQL 上不支持的功能'
            ...
            errMsg = '8.2 之前的 PostgreSQL 版本不支持的功能'
            raise SqlmapUnsupportedFeatureException(errMsg)
        ...
        try:
            ...
            errMsg = 'PostgreSQL %s 上不支持的功能（%s 位）' % (majorVer, Backend.getArch())
            raise SqlmapUnsupportedFeatureException(errMsg)
    ...
    def udfCreateFromSharedLib(self, udf, inpRet):
        if udf in self.udfToCreate:
            ...
            # 参考号：http://www.postgresql.org/docs/8.3/interactive/sql-createfunction.html
    ...
    def copyExecCmd(self, cmd):
        ...
        if isStackingAvailable():
            # 参考号：https://medium.com/greenwolf-security/authenticated-arbitrary-command-execution-on-postgresql-9-3-latest-cd18945914d5
