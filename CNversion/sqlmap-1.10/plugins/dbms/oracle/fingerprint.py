...
class Fingerprint(GenericFingerprint):
    ...
    def checkDbms(self):
        ...
        # 注意： SELECT LENGTH(SYSDATE)=LENGTH(SYSDATE) FROM DUAL 确实
        # 无法直接连接到 Oracle 数据库
        if conf.direct:
            result = True
        else:
        ...
        if result:
            ...
            # 注意： SELECT NVL(RAWTOHEX([RANDNUM1]),[RANDNUM1])=RAWTOHEX([RANDNUM1]) FROM DUAL 确实
            # 无法直接连接到 Oracle 数据库
            if conf.direct:
                result = True
            else:
            ...
            if not result:
                warnMsg = '后端数据库管理系统 不是 %s' % DBMS.ORACLE
            ...
            # 参考号：https://en.wikipedia.org/wiki/Oracle_Database
