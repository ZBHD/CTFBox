...
def forgeHeaders(items=None, base=None):
    ...
    for key, value in _.items():
        ...
        if key.upper() not in (_.upper() for _ in getPublicTypeMembers(HTTP_HEADER, True)):
            try:
                headers[_str(key)] = value  # http://bugs.python.org/issue12455 的肮脏黑客攻击
            except UnicodeEncodeError:      # 不要对非 ASCII 请求头名称进行 hack（稍后必须对它们进行正确编码）
                pass
            else:
                success = True
        if not success:
            ...
            for cookie in conf.cj:
                ...
                if ("%s=" % getUnicode(cookie.name)) in getUnicode(headers[HTTP_HEADER.COOKIE]):
                    if conf.loadCookies:
                        ...
                        message = '您提供了 HTTP %s 请求头值，而 ' % HTTP_HEADER.COOKIE
                        message += '目标 URL 在其中提供自己的 cookie '
                        message += 'HTTP %s 请求头与您的相交。 ' % HTTP_HEADER.SET_COOKIE
                        message += '您想在进一步的请求中合并它们吗？ [是/否] '
...
def checkCharEncoding(encoding, warn=True):
    ...
    # 参考号：http://www.destructor.de/charsets/index.htm
    ...
    if "8858" in encoding:
        ...
        encoding = encoding[len("x-"):]              # x-euc-kr -> euc-kr / x-mac-土耳其语 -> mac-土耳其语
    ...
    # 参考号：http://philip.html5.org/data/charsets-2.html
    if encoding in translate:
        encoding = translate[encoding]
    elif encoding in ("null", "{charset}", "charset", "*") or not re.search(r"\w", encoding):
        return None

    # 参考号：http://www.iana.org/assignments/character-sets
    # 参考号：http://docs.python.org/library/codecs.html
    try:
...
def decodePage(page, contentEncoding, contentType, percentDecode=True):
    ...
    if contentEncoding in ("gzip", "x-gzip", "deflate"):
        ...
        try:
            if contentEncoding == "deflate":
                data = io.BytesIO(zlib.decompress(page, -15))  # 参考号：http://stackoverflow.com/questions/1089662/python-inflate-and-deflate-implementations
            else:
                data = gzip.GzipFile("", "rb", 9, io.BytesIO(page))
                size = struct.unpack("<l", page[-4:])[0]  # 参考号：http://pydoc.org/get.cgi/usr/local/lib/python2.5/gzip.py
                if size > MAX_CONNECTION_TOTAL_SIZE:
                    raise Exception('尺寸太大')

            page = data.read()
        except Exception as ex:
            if b"<html" not in page:  # 在某些情况下，纯 HTML 会出现无效的“Content-Encoding”（应忽略）
                errMsg = '检测到声明内容的无效数据 '
    ...
    if not conf.encoding:
        ...
        # 参考号：http://stackoverflow.com/questions/1020892/python-urllib2-read-to-unicode
        ...
        if (any((httpCharset, metaCharset)) and (not all((httpCharset, metaCharset)) or isinstance(page, six.binary_type) and all(_ in PRINTABLE_BYTES for _ in page))) or (httpCharset == metaCharset and all((httpCharset, metaCharset))):
            kb.pageEncoding = httpCharset or metaCharset  # 参考号：http://bytes.com/topic/html-css/answers/154758-http-equiv-vs-true-header-has-precedence
        ...
        if not kb.disableHtmlDecoding:
            ...
            if percentDecode:
                if b"%" in page:
                    ...
                    page = re.sub(b"%([0-9A-F]{2})", lambda _: decodeHex(_.group(1)), page)     # 注意：CACHE 中的 %DeepSee_SQL
            ...
            if (kb.pageEncoding or "").lower() == "utf-8-sig":
                ...
                if page and page.startswith(b"\xef\xbb\xbf"):  # 参考：https://docs.python.org/2/library/codecs.html（注：注意到“utf-8-sig”留给Python处理时的问题）
                    page = page[3:]
...
def processResponse(page, responseHeaders, code=None, status=None):
    ...
    if conf.parseErrors:
        ...
        if msg:
            logger.warning('解析的 DBMS 错误消息：“%s”' % msg.rstrip('.'))
                ...
                if identYwaf.non_blind_check(rawResponse, silent=True):
                    for waf in set(identYwaf.non_blind):
                        if waf not in kb.identifiedWafs:
                            ...
                            errMsg = 'WAF/IPS 标识为“%s”' % identYwaf.format_name(waf)
                            singleTimeLogMessage(errMsg, logging.CRITICAL)
            except Exception as ex:
                singleTimeWarnMessage('WAF/IPS 检测中发生内部错误（“%s”）' % getSafeExString(ex))
