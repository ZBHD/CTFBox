...
def setDaemon(thread):
    # 参考号：http://stackoverflow.com/questions/190010/daemon-threads-explanation
    if PYVERSION >= "2.6":
        thread.daemon = True
    else:
...
def runThreads(numThreads, threadFunction, cleanupFunction=None, forwardException=True, threadChoice=False, startThreadMsg=True):
    ...
    try:
        ...
        if numThreads > 1:
            logger.info('等待线程完成%s' % (" (Ctrl+C was pressed)" if isinstance(ex, KeyboardInterrupt) else ""))
        try:
            ...
            kb.multipleCtrlC = True
            raise SqlmapThreadException('用户中止（多次按下 Ctrl+C）')
