...
def _setRequestParams():
    ...
    if conf.data is not None:
        ...
        if kb.processUserMarks is None and kb.customInjectionMark in conf.data:
            message = "在 %s 中发现自定义注入标记（'%s'） " % (kb.customInjectionMark, conf.method)
            message += '身体。您想处理吗？ [是/否/q] '
        ...
        if re.search(JSON_RECOGNITION_REGEX, conf.data):
            message = '在 %s 正文中找到 JSON 数据。 ' % conf.method
            message += '您想处理吗？ [是/否/q] '
            choice = readInput(message, default='Y').upper()
                ...
                if not (kb.processUserMarks and kb.customInjectionMark in conf.data):
                    ...
                    for match in re.finditer(r'(?P<name>[^"]+)"\s*:\s*\[([^\]]+)\]', conf.data):
                        if not (conf.testParameter and match.group("name") not in conf.testParameter):
                            _ = match.group(2)
                            if kb.customInjectionMark not in _:  # 注意：仅适用于未处理的（简单）形式 - 即非关联数组（例如 [1,2,3]）
            ...
            message = '%s 正文中发现类似 JSON 的数据。 ' % conf.method
            message += '您想处理吗？ [是/否/q] '
            choice = readInput(message, default='Y').upper()
            ...
            message = '%s 体内发现类似数组的数据。 ' % conf.method
            message += '您想处理吗？ [是/否/q] '
            choice = readInput(message, default='Y').upper()
            ...
            message = '在 %s 正文中找到 SOAP/XML 数据。 ' % conf.method
            message += '您想处理吗？ [是/否/q] '
            choice = readInput(message, default='Y').upper()
            ...
            message = '%s 体内发现类似多部分的数据。 ' % conf.method
            message += '您想处理吗？ [是/否/q] '
            choice = readInput(message, default='Y').upper()
        ...
        if not kb.postHint:
            if kb.customInjectionMark in conf.data:  # 稍后处理
                pass
            else:
                place = PLACE.POST
    ...
    if re.search(URI_INJECTABLE_REGEX, conf.url, re.I) and not any(place in conf.parameters for place in (PLACE.GET, PLACE.POST)) and not kb.postHint and kb.customInjectionMark not in (conf.data or "") and conf.url.startswith("http"):
        warnMsg = '您提供了目标 URL，但没有任何 GET '
        warnMsg += '参数（例如“http://www.site.com/article.php?id=1”） '
        warnMsg += '并且不提供任何 POST 参数 '
        warnMsg += '通过选项“--data”'
        logger.warning(warnMsg)

        message = '你想尝试 URI 注入吗 '
        message += '在目标 URL 本身中？ [是/否/q] '
        choice = readInput(message, default='Y').upper()
    ...
    for place, value in ((PLACE.URI, conf.url), (PLACE.CUSTOM_POST, conf.data), (PLACE.CUSTOM_HEADER, str(conf.httpHeaders))):
        ...
        if kb.customInjectionMark in _:
            if kb.processUserMarks is None:
                ...
                message += '“%s”。您想处理吗？ [是/否/q] ' % lut[place]
                choice = readInput(message, default='Y').upper()
            ...
            if not kb.processUserMarks:
                ...
                if place == PLACE.URI:
                    value = conf.url = conf.url.replace('+', "%20")  # 注：https://github.com/sqlmapproject/sqlmap/issues/5123
    ...
    if conf.httpHeaders:
        for httpHeader, headerValue in list(conf.httpHeaders):
            # 应避免对请求头值进行 URL 编码
            # 参考号：http://stackoverflow.com/questions/5085904/is-ok-to-urlencode-the-value-in-headerlocation-value
    ...
    if not conf.parameters:
        errMsg = '您没有提供任何 GET、POST 和 Cookie '
        errMsg += '参数，既不是 User-Agent、Referer 也不是 Host 请求头值'
        raise SqlmapGenericException(errMsg)

    elif not testableParameters:
        errMsg = '您提供的所有可测试参数都不存在 '
        errMsg += '在给定的请求数据内'
        raise SqlmapGenericException(errMsg)
    ...
    if conf.csrfToken:
        if not any(re.search(conf.csrfToken, ' '.join(_), re.I) for _ in (conf.paramDict.get(PLACE.GET, {}), conf.paramDict.get(PLACE.POST, {}), conf.paramDict.get(PLACE.COOKIE, {}))) and not re.search(r"\b%s\b" % conf.csrfToken, conf.data or "") and conf.csrfToken not in set(_[0].lower() for _ in conf.httpHeaders) and conf.csrfToken not in conf.paramDict.get(PLACE.COOKIE, {}) and not all(re.search(conf.csrfToken, _, re.I) for _ in conf.paramDict.get(PLACE.URI, {}).values()):
            errMsg = '反 CSRF 令牌参数“%s”不存在 ' % conf.csrfToken._original
            errMsg += '在提供的 GET、POST、Cookie 或请求头值中找到'
            raise SqlmapGenericException(errMsg)
    else:
        for place in (PLACE.GET, PLACE.POST, PLACE.COOKIE):
            ...
            for parameter in conf.paramDict.get(place, {}):
                if any(parameter.lower().count(_) for _ in CSRF_TOKEN_PARAMETER_INFIXES):
                    ...
                    message += '您是否希望 sqlmap 在以后的请求中自动更新它？ [是/否] '
...
def _resumeDBMS():
    ...
    if conf.dbms:
        ...
        if not check:
            message = '您提供“%s”作为后端数据库管理系统， ' % conf.dbms
            message += '但从过去扫描的目标URL信息来看 '
            message += 'sqlmap 假定后端数据库管理系统 为“%s”。 ' % dbms
            message += '你真的要强制后端吗 '
            message += '数据库管理系统的价值？ [是/否] '
...
def _resumeOS():
    ...
    if os and os != 'None':
        infoMsg = '恢复后端数据库管理系统 操作系统“%s” ' % os
        logger.info(infoMsg)

        if conf.os and conf.os.lower() != os.lower():
            message = '您提供“%s”作为后端数据库管理系统 操作 ' % conf.os
            message += '系统，但从过去的扫描信息来看 '
            message += '目标 URL sqlmap 假定后端数据库管理系统 '
            message += '操作系统是%s。 ' % os
            message += '你真的想强制后端数据库管理系统吗 '
            message += '操作系统价值？ [是/否] '
    ...
    if not conf.resultsFP:
        ...
        try:
            ...
            try:
                ...
                warnMsg += '使用临时文件“%s”代替' % conf.resultsFile
                logger.warning(warnMsg)
            except IOError as _:
                errMsg = "无法写入临时目录（'%s'）。 " % _
                errMsg += '请确保您的磁盘未满并且 '
...
def _createTargetDirs():
    ...
    try:
        ...
        warnMsg = "保存目标数据时出现问题（'%s'）" % getSafeExString(ex)
        logger.warning(warnMsg)
