...
class Databases(object):
    ...
    def getCurrentDb(self):
        ...
        if Backend.getIdentifiedDbms() in (DBMS.ORACLE, DBMS.DB2, DBMS.PGSQL, DBMS.MONETDB, DBMS.DERBY, DBMS.VERTICA, DBMS.PRESTO, DBMS.MIMERSQL, DBMS.CRATEDB, DBMS.CACHE, DBMS.FRONTBASE):
            warnMsg = '在 %s 上您需要使用 ' % Backend.getIdentifiedDbms()
            warnMsg += '枚举的模式名称作为数据库的对应项 '
            warnMsg += '其他 DBMS 上的名称'
            singleTimeWarnMessage(warnMsg)
        elif Backend.getIdentifiedDbms() in (DBMS.ALTIBASE, DBMS.CUBRID):
            warnMsg = '在 %s 上您需要使用 ' % Backend.getIdentifiedDbms()
            warnMsg += '用于枚举的用户名作为数据库的对应项 '
            warnMsg += '其他 DBMS 上的名称'
            singleTimeWarnMessage(warnMsg)
    ...
    def getDbs(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            warnMsg = 'information_schema 不可用， '
            warnMsg += '后端数据库管理系统是MySQL<5.数据库 '
            warnMsg += '名称将从“mysql”数据库中获取'
            ...
            warnMsg = '架构名称将用于 %s ' % Backend.getIdentifiedDbms()
            warnMsg += '用于枚举作为数据库的对应项 '
            warnMsg += '其他 DBMS 上的名称'
            logger.warning(warnMsg)
            ...
            warnMsg = '用户名将用于 %s ' % Backend.getIdentifiedDbms()
            warnMsg += '用于枚举作为数据库的对应项 '
            warnMsg += '其他 DBMS 上的名称'
            logger.warning(warnMsg)
    ...
    def getTables(self, bruteForce=None):
        ...
        if bruteForce is None:
            if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
                ...
                if not tables:
                    warnMsg = '无法检索表名， '
                    warnMsg += '后端数据库管理系统是%s' % Backend.getIdentifiedDbms()
                    logger.warning(warnMsg)
                    bruteForce = True
                else:
                    return tables
        ...
        if bruteForce:
            ...
            message = '您想使用公用表存在检查吗？ %s ' % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
            ...
            for db in dbs:
                ...
                for _query, _count in ((rootQuery.blind.query, rootQuery.blind.count), (getattr(rootQuery.blind, "query2", None), getattr(rootQuery.blind, "count2", None))):
                    ...
                    if count == 0:
                        ...
                        warnMsg = '无法检索号码 '
                        warnMsg += '数据库“%s”的表' % unsafeSQLIdentificatorNaming(db)
                        singleTimeWarnMessage(warnMsg)
                        continue
                    ...
                    if tables:
                        ...
                        warnMsg = '无法检索表名 '
                        warnMsg += '对于数据库“%s”' % unsafeSQLIdentificatorNaming(db)
                        logger.warning(warnMsg)
    ...
    def getColumns(self, onlyColNames=False, colTuple=None, bruteForce=None, dumpMode=False):
        ...
        if bruteForce:
            ...
            if kb.choices.columnExists is None:
                message = '您想使用公共列存在检查吗？ %s' % ("[Y/n/q]" if Backend.getIdentifiedDbms() in (DBMS.ACCESS, DBMS.MCKOI, DBMS.EXTREMEDB) else "[y/N/q]")
        ...
        if any(isTechniqueAvailable(_) for _ in (PAYLOAD.TECHNIQUE.UNION, PAYLOAD.TECHNIQUE.ERROR, PAYLOAD.TECHNIQUE.QUERY)) or conf.direct:
            for tbl in tblList:
                ...
                if dumpMode and colList:
                    ...
                    infoMsg += '对于表“%s” ' % unsafeSQLIdentificatorNaming(tbl)
                    if METADB_SUFFIX not in conf.db:
                            ...
                            if name:
                                if conf.getComments:
                                    ...
                                    if hasattr(_, "query"):
                                        ...
                                        comment = unArrayizeValue(inject.getValue(query, blind=False, time=False))
                                        if not isNoneValue(comment):
                                            infoMsg = '检索到列“%s”的注释“%s”' % (comment, name)
                                            logger.info(infoMsg)
                                    else:
                                        warnMsg = '在 %s 上不是 ' % Backend.getIdentifiedDbms()
                                        warnMsg += '可以获得专栏评论'
            ...
            for tbl in tblList:
                ...
                if dumpMode and colList:
                    ...
                    if not isNumPosStrValue(count):
                        ...
                        if not columns:
                            errMsg = '无法检索 %s 列 ' % ("number of " if not Backend.isDbms(DBMS.MSSQL) else "")
                            errMsg += '对于表“%s” ' % unsafeSQLIdentificatorNaming(tbl)
                            if METADB_SUFFIX not in conf.db:
                                errMsg += '在数据库“%s”中' % unsafeSQLIdentificatorNaming(conf.db)
                            logger.error(errMsg)
                            continue
                    ...
                    if not isNoneValue(column):
                        if conf.getComments:
                            ...
                            if hasattr(_, "query"):
                                ...
                                comment = unArrayizeValue(inject.getValue(query, union=False, error=False))
                                if not isNoneValue(comment):
                                    infoMsg = '检索到列“%s”的注释“%s”' % (comment, column)
                                    logger.info(infoMsg)
                            else:
                                warnMsg = '在 %s 上不是 ' % Backend.getIdentifiedDbms()
                                warnMsg += '可以获得专栏评论'
        ...
        if not kb.data.cachedColumns:
            warnMsg = '无法检索列名 '
            warnMsg += ("table '%s' " % unsafeSQLIdentificatorNaming(unArrayizeValue(tblList))) if len(tblList) == 1 else '任何桌子 '
            if METADB_SUFFIX not in conf.db:
    ...
    def getCount(self):
        ...
        if conf.tbl is not None and conf.db is None and Backend.getIdentifiedDbms() not in (DBMS.SQLITE, DBMS.ACCESS, DBMS.FIREBIRD, DBMS.MCKOI, DBMS.EXTREMEDB):
            warnMsg = '缺少数据库参数。 sqlmap 将会 '
            warnMsg += '使用当前数据库检索 '
            warnMsg += '表“%s”的条目数' % unsafeSQLIdentificatorNaming(conf.tbl)
            logger.warning(warnMsg)
    ...
    def getStatements(self):
        ...
        if not kb.data.cachedStatements and isInferenceAvailable() and not conf.direct:
            ...
            for index in indexRange:
                ...
                if Backend.getIdentifiedDbms() in (DBMS.MYSQL,):  # 多进程的情况
