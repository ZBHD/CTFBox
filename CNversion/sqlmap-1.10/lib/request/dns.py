...
class DNSQuery(object):
    ...
    def response(self, resolution):
        ...
        if self._query:
            retVal += self._raw[:2]                                                         # 交易ID
            retVal += b"\x85\x80"                                                           # 标志（标准查询响应，无错误）
            retVal += self._raw[4:6] + self._raw[4:6] + b"\x00\x00\x00\x00"                 # 问题和答案很重要
            retVal += self._raw[12:(12 + self._raw[12:].find(b"\x00") + 5)]                 # 原始域名查询
            retVal += b"\xc0\x0c"                                                           # 指向域名的指针
            retVal += b"\x00\x01"                                                           # Type A
            retVal += b"\x00\x01"                                                           # 中级
            retVal += b"\x00\x00\x00\x20"                                                   # TTL（32 秒）
            retVal += b"\x00\x04"                                                           # 数据长度
            retVal += b"".join(struct.pack('B', int(_)) for _ in resolution.split('.'))     # 4字节IP
...
class DNSServer(object):
    ...
    def _check_localhost(self):
        ...
        try:
            ...
            s.connect(("", 53))
            s.send(binascii.unhexlify("6509012000010000000000010377777706676f6f676c6503636f6d00000100010000291000000000000000"))  # 一个 www.google.com
            response = s.recv(512)
        except:
            pass
        finally:
            if response and b"google" in response:
                raise socket.error('另一个 DNS 服务已在“0.0.0.0:53”上运行')
