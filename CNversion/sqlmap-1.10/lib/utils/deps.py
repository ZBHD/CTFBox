...
def checkDependencies():
        ...
        try:
            if dbmsName in (DBMS.MSSQL, DBMS.SYBASE):
                ...
                if not hasattr(pymssql, "__version__") or pymssql.__version__ < "1.0.2":
                    ...
                    warnMsg += '版本 >= 1.0.2 才能正常工作。 '
                    warnMsg += '从“%s”下载' % data[2]
            ...
            warnMsg += '为了直接连接到DBMS '
            warnMsg += '“%s”。从“%s”下载' % (dbmsName, data[2])
            logger.warning(warnMsg)
            missing_libraries.add(data[1])

            continue

        debugMsg = '找到“%s”第三方库' % data[1]
        logger.debug(debugMsg)

    try:
        __import__("impacket")
        debugMsg = '找到“python-impacket”第三方库'
        logger.debug(debugMsg)
    except ImportError:
        warnMsg = 'sqlmap 需要“python-impacket”第三方库 '
        warnMsg += '带外接管功能。下载自 '
    ...
    try:
        __import__("ntlm")
        debugMsg = '找到“python-ntlm”第三方库'
        logger.debug(debugMsg)
    except ImportError:
        warnMsg = "sqlmap需要'python-ntlm'第三方库 "
        warnMsg += '如果您计划攻击 NTLM 背后的 Web 应用程序 '
        warnMsg += '验证。从“https://github.com/mullender/python-ntlm”下载'
        logger.warning(warnMsg)
        missing_libraries.add('python-ntlm')

    try:
        __import__("httpx")
        debugMsg = "找到'httpx[http2]'第三方库"
        logger.debug(debugMsg)
    except ImportError:
        warnMsg = "sqlmap需要'httpx[http2]'第三方库 "
        warnMsg += '如果您计划使用 HTTP 版本 2'
        logger.warning(warnMsg)
        missing_libraries.add('httpx[http2]')

    try:
        __import__("websocket._abnf")
        debugMsg = '找到“websocket-client”库'
        logger.debug(debugMsg)
    except ImportError:
        warnMsg = "sqlmap需要'websocket-client'第三方库 "
        warnMsg += '如果您计划使用 WebSocket 攻击 Web 应用程序。 '
        warnMsg += '从“https://pypi.python.org/pypi/websocket-client/”下载'
        logger.warning(warnMsg)
        missing_libraries.add('websocket-client')

    try:
        __import__("tkinter")
        debugMsg = '找到“tkinter”库'
        logger.debug(debugMsg)
    except ImportError:
        warnMsg = 'sqlmap 需要“tkinter”库 '
        warnMsg += '如果您打算运行 GUI'
        logger.warning(warnMsg)
    ...
    try:
        ...
        debugMsg = '找到“tkinter.ttk”库'
        logger.debug(debugMsg)
    except ImportError:
        warnMsg = 'sqlmap 需要“tkinter.ttk”库 '
        warnMsg += '如果您打算运行 GUI'
        logger.warning(warnMsg)
    ...
    if IS_WIN:
        try:
            ...
            debugMsg = '找到“python-pyreadline”第三方库'
            logger.debug(debugMsg)
        except ImportError:
            warnMsg = "sqlmap需要'pyreadline'第三方库来 "
            warnMsg += '能够利用 sqlmap TAB '
            warnMsg += 'SQL 中的完成和历史支持功能 '
            warnMsg += '外壳和操作系统外壳。下载自 '
