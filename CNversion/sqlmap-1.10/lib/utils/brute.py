...
def tableExists(tableFile, regex=None):
    ...
    message = '您想使用哪个通用表（单词列表）文件？\n'
    message += '[1] 默认“%s”（按 Enter）\n' % tableFile
    message += "[2] custom"
    choice = readInput(message, default='1')

    if choice == '2':
        message = '自定义公用表文件位置是什么？\n'
        tableFile = readInput(message) or tableFile

    infoMsg = '使用“%s”中的项目执行表存在' % tableFile
    logger.info(infoMsg)
        ...
        def tableExistsThread():
            ...
            while kb.threadContinue:
                ...
                if conf.verbose in (1, 2):
                    ...
                    dataToStdout('\r[%s] [INFO] 尝试过 %s' % (time.strftime("%X"), status), True)
...
def columnExists(columnFile, regex=None):
    ...
    message = '您想使用哪个常用列（单词列表）文件？\n'
    message += '[1] 默认“%s”（按 Enter）\n' % columnFile
    message += "[2] custom"
    choice = readInput(message, default='1')

    if choice == '2':
        message = '自定义公共列文件位置是什么？\n'
        columnFile = readInput(message) or columnFile

    infoMsg = '使用“%s”中的项目检查列是否存在' % columnFile
    logger.info(infoMsg)
...
def fileExists(pathFile):
    ...
    if choice == '2':
        message = '自定义通用文件的文件位置是什么？\n'
        pathFile = readInput(message) or pathFile

    infoMsg = '使用“%s”中的项目检查文件是否存在' % pathFile
    logger.info(infoMsg)
