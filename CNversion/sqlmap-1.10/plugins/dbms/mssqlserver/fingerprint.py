...
class Fingerprint(GenericFingerprint):
    ...
    def checkDbms(self):
        ...
        # 注意： SELECT LEN(@@VERSION)=LEN(@@VERSION) FROM DUAL 不会
        # 直接连接到 Microsoft SQL Server 数据库
        if conf.direct:
            result = True
        else:
        ...
        if result:
            ...
            warnMsg = '后端数据库管理系统 不是 %s' % DBMS.MSSQL
    ...
    def checkDbmsOs(self, detailed=False):
        ...
        # 参考号：https://en.wikipedia.org/wiki/Comparison_of_Microsoft_Windows_versions
        ...
        if not Backend.getOsVersion():
            ...
            warnMsg = '无法识别底层操作 '
            warnMsg += '系统版本，假设是Windows '
            warnMsg += '%s 服务包 %d' % (Backend.getOsVersion(), Backend.getOsServicePack())
            logger.warning(warnMsg)
