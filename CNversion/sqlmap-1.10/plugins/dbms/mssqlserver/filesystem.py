...
class Filesystem(GenericFilesystem):
    ...
    def _updateDestChunk(self, fileContent, tmpPath):
        ...
        logger.debug('正在上传调试脚本到%s\\%s，请稍候..' % (tmpPath, randScr))

        self.xpCmdshellWriteFile(fileScrLines, tmpPath, randScr)

        logger.debug('从调试脚本 %s 生成块文件 %s\\%s' % (tmpPath, chunkName, randScr))
    ...
    def stackedReadFile(self, remoteFile):
        ...
        logger.debug('将文件“%s”的内容加载到支持表中' % remoteFile)
        ...
        # 参考号：https://web.archive.org/web/20120211184457/http://support.microsoft.com/kb/104829
    ...
    def unionWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        errMsg = 'Microsoft SQL Server 不支持文件上传 '
        errMsg += "UNION query SQL injection technique"
        raise SqlmapUnsupportedFeatureException(errMsg)

    def _stackedWriteFilePS(self, tmpPath, localFileContent, remoteFile, fileType):
        infoMsg = '使用PowerShell写入%s文件内容 ' % fileType
        infoMsg += '归档“%s”' % remoteFile
        logger.info(infoMsg)
        ...
        logger.debug('正在上传base64编码文件到%s，请稍候..' % encodedBase64FilePath)
        ...
        logger.debug('将PowerShell base64解码脚本上传到%s' % randPSScriptPath)
        self.xpCmdshellWriteFile(psString, tmpPath, randPSScript)

        logger.debug('正在执行PowerShell base64解码脚本来写入%s文件，请稍候..' % remoteFile)
    ...
    def _stackedWriteFileDebugExe(self, tmpPath, localFile, localFileContent, remoteFile, fileType):
        infoMsg = '使用debug.exe写入%s ' % fileType
        infoMsg += '文件内容到文件“%s”，请稍候..' % remoteFile
        logger.info(infoMsg)
        ...
        if localFileSize < debugSize:
            ...
            debugMsg = '将块文件 %s\\%s 重命名为 %s ' % (tmpPath, chunkName, fileType)
            ...
            debugMsg = '文件大于 %d 字节。 ' % debugSize
            debugMsg += 'sqlmap会在本地将其分割成块，上传 '
            debugMsg += '它逐块并重新创建原始文件 '
            debugMsg += '正在服务器中，请稍等..'
            logger.debug(debugMsg)
    ...
    def _stackedWriteFileVbs(self, tmpPath, localFileContent, remoteFile, fileType):
        infoMsg = '使用自定义的 Visual Basic 脚本来编写 '
        infoMsg += '%s 文件内容已写入文件“%s”，请稍候..' % (fileType, remoteFile)
        logger.info(infoMsg)
        ...
        # 注：https://github.com/sqlmapproject/sqlmap/issues/5581
        vbs = codecs.decode(vbs, "rot13")
        vbs = vbs.replace("    ", "")
        encodedFileContent = encodeBase64(localFileContent, binary=False)

        logger.debug('正在将文件base64编码的内容上传到%s，请稍候..' % randFilePath)

        self.xpCmdshellWriteFile(encodedFileContent, tmpPath, randFile)

        logger.debug('正在上传 Visual Basic 解码器存根 %s\\%s，请稍候..' % (tmpPath, randVbs))
    ...
    def _stackedWriteFileCertutilExe(self, tmpPath, localFile, localFileContent, remoteFile, fileType):
        infoMsg = '使用 certutil.exe 写入 %s ' % fileType
        infoMsg += '文件内容到文件“%s”，请稍候..' % remoteFile
        logger.info(infoMsg)
    ...
    def stackedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        # 注意：这里需要这是因为我们使用 xp_cmdshell 扩展
        # 在后端 Microsoft SQL Server 上写入文件的过程
        # 文件系统
        self.initEnv()
        ...
        if written is False:
            message = '你想尝试上传文件吗 '
            message += '自定义 Visual Basic 脚本技术？ [是/否] '
        ...
        if written is False:
            message = '你想尝试上传文件吗 '
            message += '内置的 certutil.exe 技术？ [是/否] '
