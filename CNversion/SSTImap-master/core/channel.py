...
class Channel:
    ...
    def req(self, injection):
        ...
        if self.tag in self.base_url:
            log.debug(f'[路径] {url_params}')
        if get_params:
            log.debug(f'[查询参数] {get_params}')
        if post_params:
            log.debug(f'[请求体] {post_params}')
        if len(header_params) > 1:
            log.debug(f'[请求头] {header_params}')
        ...
        try:
            ...
            if e and e.args[0] and e.args[0].args[0] == 'Connection aborted.':
                ...
                log.log(25, f'错误：连接失败： {repr(e)}')
                result = ("", 0.0, {})
        except requests.exceptions.RequestException as e:
                log.log(25, f'错误：请求失败： {repr(e)}')
                result = ("", 0.0, {})
        except urllib3.exceptions.HTTPError as e:
                log.log(25, f'错误：{repr(e)}')
                result = ("", 0.0, {})
        else:
