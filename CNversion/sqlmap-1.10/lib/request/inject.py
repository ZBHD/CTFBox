        ...
        if (conf.eta or conf.threads > 1) and Backend.getIdentifiedDbms() and not re.search(r"(COUNT|LTRIM)\(", expression, re.I) and not (timeBasedCompare and not kb.forceThreads):
            ...
            if field and re.search(r"\ASELECT\s+DISTINCT\((.+?)\)\s+FROM", expression, re.I):
                if Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.PGSQL, DBMS.MONETDB, DBMS.VERTICA, DBMS.CRATEDB, DBMS.CUBRID):
                    ...
                    expression = "SELECT %s FROM (%s)" % (field if '.' not in field else re.sub(r".+\.", "%s." % alias, field), expression)  # 注意：MonetDB 作为一个主要示例
...
def _goInferenceProxy(expression, fromUser=False, batch=False, unpack=True, charsetType=None, firstChar=None, lastChar=None, dump=False):
    ...
    if len(expressionFieldsList) > 1:
        infoMsg = '提供的 SQL 查询有多个字段。 '
        infoMsg += 'sqlmap 现在会将其解压缩为不同的查询 '
        infoMsg += '即使我们能够检索输出 '
        infoMsg += '正在失明'
        logger.info(infoMsg)

    # 如果我们从 SQL 查询/shell 到达这里，我们必须检查是否
    # SQL 查询可能会返回多个条目，在这种情况下
    # 伪造 SQL 限制查询输出一次一项
    # 注意：我们假设只有从表中获取数据的查询
    # 可以返回多个条目
                ...
                if not stopLimit:
                    ...
                    if isNumPosStrValue(count):
                        ...
                        if batch or count == 1:
                            ...
                            message = '提供的 SQL 查询可以返回 '
                            message += '%d 条目。多少 ' % count
                            message += '您想要检索条目吗？\n'
                            message += '[a]全部（默认）\n[#]具体数量\n'
                            message += "[q] Quit"
                            ...
                            if choice == 'A':
                                ...
                                infoMsg = 'sqlmap 现在将检索 '
                                infoMsg += '第一个 %d 查询输出条目' % stopLimit
                                logger.info(infoMsg)
...
def getValue(expression, blind=True, union=True, error=True, time=True, fromUser=False, expected=None, batch=False, unpack=True, resumeValue=True, charsetType=None, firstChar=None, lastChar=None, dump=False, suppressOutput=None, expectingNone=False, safeCharEncode=True):
    ...
    if conf.hexConvert and expected != EXPECTED.BOOL and Backend.getIdentifiedDbms():
        if not hasattr(queries[Backend.getIdentifiedDbms()], "hex"):
            warnMsg = 'DBMS %s 目前不支持开关“--hex”' % Backend.getIdentifiedDbms()
        ...
        if conf.direct:
            ...
            if not conf.forceDns:
                if union and isTechniqueAvailable(PAYLOAD.TECHNIQUE.UNION):
                    ...
                    if expected == EXPECTED.BOOL:
                        # 注意：某些 DBMS（例如 Altibase）不支持在前缀和后缀连接期间布尔检查结果的隐式转换（例如 'qjjvq'||(1=1)||'qbbbq'）
                    ...
                    if not found and fallback:
                        warnMsg = '完整的 UNION 出了点问题 '
                        warnMsg += '技术（可能是因为 '
                        warnMsg += '检索条目数量的限制）'
                        if " FROM " in query.upper():
                            warnMsg += '。回到部分 UNION 技术'
                            singleTimeWarnMessage(warnMsg)
    ...
    if not any((kb.testMode, conf.dummy, conf.offline, conf.noCast, conf.hexConvert)) and value is None and Backend.getDbms() and conf.dbmsHandler and kb.fingerprinted:
        if conf.abortOnEmpty:
            ...
            warnMsg = '如果出现连续数据检索问题，建议您尝试 '
            warnMsg += '开关“--no-cast” '
            warnMsg += '或切换“--hex”' if hasattr(queries[Backend.getIdentifiedDbms()], "hex") else ""
            singleTimeWarnMessage(warnMsg)

    # 脏补丁（MSSQL --binary-fields 带有 0x31003200...）
