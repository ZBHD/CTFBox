...
try:
    ...
    try:
        __import__("lib.utils.versioncheck")  # 这必须是第一个非标准导入
    except ImportError:
...
def checkEnvironment():
    try:
        ...
        errMsg = '您的系统无法正确处理非 ASCII 路径。 '
        errMsg += '请将sqlmap的目录移动到其他位置'
        logger.critical(errMsg)
        raise SystemExit

    if LooseVersion(VERSION) < LooseVersion("1.0"):
        errMsg = '您的运行时环境（例如 PYTHONPATH）是 '
        errMsg += '破碎的。请确保您没有跑步 '
        errMsg += '较新版本的 sqlmap 以及较旧版本的运行时脚本 '
        errMsg += "versions"
        logger.critical(errMsg)
        raise SystemExit
...
def main():
    ...
    try:
                        ...
                        if conf.crawlDepth and conf.bulkFile:
                            ...
                            for i in xrange(len(targets)):
                                ...
                                try:
                                    ...
                                    infoMsg = '正在启动目标 URL“%s”的爬取程序 (%d/%d)' % (target, i + 1, len(targets))
                                    logger.info(infoMsg)

                                    crawl(target)
                                except Exception as ex:
                                    if target and not isinstance(ex, SqlmapUserQuitException):
                                        errMsg = '抓取“%s”（“%s”）时出现问题' % (target, getSafeExString(ex))
                                        logger.error(errMsg)
                                    else:
                                        raise
                                else:
        ...
        if any(_ in excMsg for _ in ("MemoryError", "Cannot allocate memory")):
            ...
            errMsg = "无效网址（'%s'）" % excMsg.strip().split('\n')[-1]
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = '访问文件“%s”时发生权限错误' % match.group(1)
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = '请更新“httpcore”包（>= 1.0.8） '
            errMsg += "（参考：'https://github.com/encode/httpcore/discussions/995'）"
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = '使用 ALPHA 版本的 Python 3.11 运行 sqlmap 时存在一个已知问题。 '
            errMsg += '请下载稳定的Python版本'
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = '检测到来自不同版本 sqlmap 的文件的使用情况'
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = '代码校验和失败（关闭自动问题创建）。 '
            errMsg += '您应该从官方 GitHub 获取最新的开发版本 '
            errMsg += '存储库位于“%s”' % GIT_PAGE
            logger.critical(errMsg)
            print()
            dataToStdout(excMsg)
            raise SystemExit
            ...
            errMsg = "无效的运行时环境（'%s'）" % excMsg.split("Error: ")[-1].strip()
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = '包“python-ntlm”与以下版本存在已知的兼容性问题 '
            errMsg += "Python 3（参考：'https://github.com/mullender/python-ntlm/pull/61'）"
            logger.critical(errMsg)
            raise SystemExit

        elif "'DictObject' object has no attribute '" in excMsg and all(_ in errMsg for _ in ("(fingerprinted)", "(identified)")):
            errMsg = '枚举出现了问题。 '
            errMsg += '由于假阳性病例的可能性相当大 '
            errMsg += '建议您使用开关“--flush-session”重新运行'
            logger.critical(errMsg)
            raise SystemExit

        elif "database disk image is malformed" in excMsg:
            errMsg = '本地会话文件似乎格式错误。请使用“--flush-session”重新运行'
            logger.critical(errMsg)
            raise SystemExit
            ...
            errMsg = '无效的运行时间（“%s”） ' % excMsg.split("Error: ")[-1].strip()
            ...
            errMsg = '您的 .pyc 文件之一已损坏%s' % (" ('%s')" % match.group(1) if match else "")
            errMsg += '。请删除系统上的 .pyc 文件来解决问题'
            logger.critical(errMsg)
            raise SystemExit
        ...
        for tempDir in conf.get("tempDirs", []):
            ...
            if any((conf.vulnTest, conf.smokeTest)) or not filterNone(filepath for filepath in glob.glob(os.path.join(tempDir, '*')) if not any(filepath.endswith(_) for _ in (".lock", ".exe", ".so", '_'))):  # 忽略垃圾文件
                try:
...
if __name__ == "__main__":
    try:
        ...
        # 参考号：http://stackoverflow.com/questions/1635080/terminate-a-multi-thread-python-program
