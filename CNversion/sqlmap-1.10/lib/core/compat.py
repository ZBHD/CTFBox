...
class WichmannHill(random.Random):
    ...
    VERSION = 1     # 由 getstate/setstate 使用
    ...
    def random(self):
        ...
        # Wichman-Hill 随机数生成器。
        #
        # Wichmann, B. A. 和 Hill, I. D. (1982)
        # 算法 AS 183：
        # 高效便携的伪随机数生成器
        # 应用统计学31（1982）188-190
        #
        # 参见：
        # 对算法 AS 183 的更正
        # 应用统计学33（1984）123
        ...
        # 注意：在使用 IEEE-754 双精度算术的平台上，这可以
        # 永远不会返回 0.0（由 Tim 断言；证明太长，无法发表评论）。
    ...
    def setstate(self, state):
        ...
        if version == 1:
            ...
            raise ValueError('版本 %s 的状态传递给版本 %s 的 Random.setstate()' %
    ...
    def __whseed(self, x=0, y=0, z=0):
        ...
        if not type(x) == type(y) == type(z) == int:
            raise TypeError('种子必须是整数')
        if not (0 <= x < 256 and 0 <= y < 256 and 0 <= z < 256):
            raise ValueError('种子必须在范围 (0, 256) 内')
        if 0 == x == y == z:
            # 从当前时间开始初始化
            t = int(time.time() * 256)
...
# 参考号：https://github.com/urllib3/urllib3/blob/master/src/urllib3/filepost.py
def choose_boundary():
    """
...
# 参考号：http://python3porting.com/differences.html
def round(x, d=0):
    """
    >>> round(2.0)
    2.0
...
# 参考号：https://code.activestate.com/recipes/576653-convert-a-cmp-function-to-a-key-function/
...
# 注意：codecs.open 重新实现（在 Python 3.14 中已弃用）
