    ...
    if conf.pivotColumn:
        for _ in colList:
            if re.search(r"(.+\.)?%s" % re.escape(conf.pivotColumn), _, re.I):
                infoMsg = '使用列“%s”作为枢轴 ' % conf.pivotColumn
                infoMsg += '用于检索行数据'
                logger.info(infoMsg)
        ...
        if not validPivotValue:
            ...
            warnMsg += '在表“%s”中找到' % table
            logger.warning(warnMsg)

    if not validPivotValue:
        for column in colList:
            infoMsg = '获取不同的数量 '
            infoMsg += '列“%s”的值' % column.replace(("%s." % alias) if alias else "", "")
            logger.info(infoMsg)
            ...
            if isNumPosStrValue(value):
                ...
                if value == count:
                    infoMsg = '使用列“%s”作为枢轴 ' % column.replace(("%s." % alias) if alias else "", "")
                    infoMsg += '用于检索行数据'
                    logger.info(infoMsg)
