...
def main():
    ...
    apiparser.add_argument("-p", "--port", help='REST-JSON API服务器的端口（默认%d）' % RESTAPI_DEFAULT_PORT, default=RESTAPI_DEFAULT_PORT, type=int)
    apiparser.add_argument("--adapter", help='要使用的服务器（瓶子）适配器（默认“%s”）' % RESTAPI_DEFAULT_ADAPTER, default=RESTAPI_DEFAULT_ADAPTER)
