...
class Entries(object):
        ...
        for tbl in tblList:
            ...
            try:
                ...
                if not kb.data.dumpedTable and isInferenceAvailable() and not conf.direct:
                    ...
                    infoMsg += '表“%s”的条目 ' % unsafeSQLIdentificatorNaming(tbl)
                    infoMsg += '在数据库“%s”中' % unsafeSQLIdentificatorNaming(conf.db)
                    logger.info(infoMsg)
                    ...
                    if count == 0:
                        ...
                        warnMsg += '表“%s”的条目 ' % unsafeSQLIdentificatorNaming(tbl)
                        warnMsg += '在数据库“%s”中' % unsafeSQLIdentificatorNaming(conf.db)
                        logger.warning(warnMsg)
                        ...
                        if len(colList) < len(indexRange) > CHECK_ZERO_COLUMNS_THRESHOLD:
                            ...
                            for column in colList:
                                if not inject.checkBooleanExpression("(SELECT COUNT(%s) FROM %s)>0" % (column, kb.dumpTable)):
                                    ...
                                    debugMsg = '表“%s”的列“%s”将不会 ' % (column, kb.dumpTable)
                                    debugMsg += '因看起来是空的而被倾倒'
                ...
                if len(kb.data.dumpedTable) == 0 or (entriesCount == 0 and kb.permissionFlag):
                    warnMsg = '无法检索条目 '
                    if conf.col:
                        warnMsg += '列数“%s” ' % colNames
                    warnMsg += '对于表“%s” ' % unsafeSQLIdentificatorNaming(tbl)
                    warnMsg += '在数据库“%s”%s' % (unsafeSQLIdentificatorNaming(conf.db), " (permission denied)" if kb.permissionFlag else "")
                    logger.warning(warnMsg)
                else:
                    ...
                    if not conf.disableHashing:
                        try:
                            ...
                            errMsg = '攻击时发生错误 '
                            errMsg += "表转储（'%s'）" % getSafeExString(ex)
    ...
    def dumpAll(self):
        ...
        infoMsg = 'sqlmap 现在将从所有数据库中转储所有表的条目'
        logger.info(infoMsg)
