...
class Database(object):
    ...
    def connect(self, who="server"):
        ...
        logger.debug('REST-JSON API %s 连接到 IPC 数据库' % who)
...
@return_error(404)  # 未找到
def error404(error=None):
    security_headers(False)
    return "Nothing here"

@return_error(405)  # 不允许的方法（例如，通过 GET 请求 POST 方法时）
...
def task_delete(taskid):
    ...
    if taskid in DataStore.tasks:
        ...
        logger.debug('(%s) 已删除的任务' % taskid)
        return jsonize({"success": True})
    else:
        response.status = 404
        logger.warning('[%s] 提供给task_delete() 的不存在的任务ID' % taskid)
...
def task_flush(token=None):
    ...
    logger.debug('(%s) 刷新任务池(%s)' % (token, "admin" if is_admin(token) else request.remote_addr))
    return jsonize({"success": True})
...
def option_list(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 option_list() 的任务 ID 无效' % taskid)
...
def option_get(taskid):
    ...
    """
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 option_get() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
    ...
    for option in options:
        if option in DataStore.tasks[taskid].options:
            ...
            logger.debug('（%s）未知选项“%s”的请求值' % (taskid, option))
            return jsonize({"success": False, "message": "Unknown option '%s'" % option})

    logger.debug('(%s) 检索到选项“%s”的值' % (taskid, ','.join(options)))
...
def option_set(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 option_set() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})

    if request.json is None:
        logger.warning('[%s] 提供给 option_set() 的 JSON 选项无效' % taskid)
...
def scan_start(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 scan_start() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})

    if request.json is None:
        logger.warning('[%s] 提供给 scan_start() 的 JSON 选项无效' % taskid)
        return jsonize({"success": False, "message": "Invalid JSON options"})

    for key in request.json:
        if key in RESTAPI_UNSUPPORTED_OPTIONS:
            logger.warning('[%s] 为 scan_start() 提供了不支持的选项“%s”' % (taskid, key))
            return jsonize({"success": False, "message": "Unsupported option '%s'" % key})

    # 使用用户提供的选项初始化 sqlmap 引擎的选项（如果有）
    for option, value in request.json.items():
        DataStore.tasks[taskid].set_option(option, value)
...
def scan_stop(taskid):
    ...
    if (taskid not in DataStore.tasks or DataStore.tasks[taskid].engine_process() is None or DataStore.tasks[taskid].engine_has_terminated()):
        logger.warning('[%s] 提供给 scan_stop() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
...
def scan_kill(taskid):
    ...
    if (taskid not in DataStore.tasks or DataStore.tasks[taskid].engine_process() is None or DataStore.tasks[taskid].engine_has_terminated()):
        logger.warning('[%s] 提供给 scan_kill() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})

    DataStore.tasks[taskid].engine_kill()

    logger.debug('(%s) 终止扫描' % taskid)
    return jsonize({"success": True})
...
def scan_status(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 scan_status() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
...
def scan_data(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 scan_data() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})

    # 从IPC数据库中读取taskid的所有数据
...
def scan_log_limited(taskid, start, end):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 scan_log_limited() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
...
def scan_log(taskid):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 scan_log() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
...
def download(taskid, target, filename):
    ...
    if taskid not in DataStore.tasks:
        logger.warning('[%s] 提供给 download() 的任务 ID 无效' % taskid)
        return jsonize({"success": False, "message": "Invalid task ID"})
    ...
    if os.path.isfile(path):
        logger.debug('(%s) 检索到文件 %s 的内容' % (taskid, target))
...
def server(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, adapter=RESTAPI_DEFAULT_ADAPTER, username=None, password=None, database=None):
    ...
    logger.info('正在“%s:%d”运行 REST-JSON API 服务器..' % (host, port))
    ...
    try:
        ...
        if adapter.lower() not in server_names:
            ...
            errMsg += '（注意：您可以尝试使用“apt install python-%s”或“pip%s install %s”来安装它）' % (adapter, '3' if six.PY3 else "", adapter)
...
def client(host=RESTAPI_DEFAULT_ADDRESS, port=RESTAPI_DEFAULT_PORT, username=None, password=None):
    ...
    dbgMsg = '从命令行进行客户端访问的示例：'
    dbgMsg += "\n\t$taskid=$(curl http://%s:%d/task/new 2>1 | grep -o -I '[a-f0-9]\\{16\\}') && echo $taskid" % (host, port)
    dbgMsg += "\n\t$curl -H“内容类型：application/json”-X POST -d '{“url”：“http://testphp.vulnweb.com/artists.php?artist=1”}'http://%s:%d/scan/$taskid/start" % (host, port)
    dbgMsg += '\n\t$ 卷曲 http://%s:%d/scan/$taskid/data' % (host, port)
    dbgMsg += '\n\t$ 卷曲 http://%s:%d/scan/$taskid/log' % (host, port)
    logger.debug(dbgMsg)

    addr = "http://%s:%d" % (host, port)
    logger.info('正在启动 REST-JSON API 客户端到“%s”...' % addr)
    ...
    try:
        ...
        if not isinstance(ex, _urllib.error.HTTPError) or ex.code == _http_client.UNAUTHORIZED:
            errMsg = '连接到时出现问题 '
            errMsg += 'REST-JSON API 服务器位于“%s” ' % addr
    ...
    while True:
        ...
        if command in ("data", "log", "status", "stop", "kill"):
            if not taskid:
                logger.error('没有正在使用的任务 ID')
                continue
            raw = _client("%s/scan/%s/%s" % (addr, taskid, command))
            res = dejsonize(raw)
            if not res["success"]:
                logger.error('执行命令%s失败' % command)
            dataToStdout("%s\n" % raw)
            ...
            try:
                ...
                logger.error("解析参数时出错（'%s'）" % getSafeExString(ex))
                taskid = None
                continue
            ...
            if not res["success"]:
                ...
                continue
            taskid = res["taskid"]
            logger.info('新任务 ID 为“%s”' % taskid)

            raw = _client("%s/scan/%s/start" % (addr, taskid), cmdLineOptions)
            res = dejsonize(raw)
            if not res["success"]:
                logger.error("无法开始扫描（'%s'）" % res.get("message", ""))
                continue
            logger.info('扫描开始')
            ...
            if not taskid:
                logger.error('任务 ID 缺失')
                taskid = None
                continue
            elif not re.search(r"\A[0-9a-fA-F]{16}\Z", taskid):
                logger.error('无效的任务 ID“%s”' % taskid)
                taskid = None
                continue
            logger.info('切换到任务 ID“%s” ' % taskid)
            ...
            msg += 'use TASKID 将当前上下文切换到不同的任务（例如“use c04d8c5c7582efb4”）\n'
