...
def purge(directory):
    ...
    if not os.path.isdir(directory):
        warnMsg = '跳过目录“%s”的清除，因为它不存在' % directory
        logger.warning(warnMsg)
        return

    infoMsg = '正在清除目录“%s”的内容...' % directory
    logger.info(infoMsg)
    ...
    try:
        ...
        logger.error('删除目录“%s”（“%s”）时出现问题' % (getUnicode(directory), getSafeExString(ex)))
