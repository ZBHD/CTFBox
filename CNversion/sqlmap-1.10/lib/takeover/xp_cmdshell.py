...
class XP_cmdshell(object):
    ...
    def _xpCmdshellConfigure2005(self, mode):
        debugMsg = '使用 sp_configure 配置 xp_cmdshell '
        debugMsg += '存储过程'
    ...
    def _xpCmdshellConfigure2000(self, mode):
        debugMsg = '使用 sp_addextendedproc 配置 xp_cmdshell '
        debugMsg += '存储过程'
        logger.debug(debugMsg)
    ...
    def _xpCmdshellTest(self):
        ...
        logger.info('测试 xp_cmdshell 扩展过程是否可用')
        output = self.xpCmdshellEvalCmd("echo 1")

        if output == "1":
            logger.info('xp_cmdshell扩展程序可用')
        elif isNoneValue(output) and conf.dbmsCred:
            errMsg = "看来临时目录（'%s'）用于 " % self.getRemoteTempPath()
            errMsg += '将控制台输出存储在后端文件系统中 '
            errMsg += '没有 DBMS 进程的写权限。 '
            errMsg += '建议您使用选项手动调整 '
            errMsg += "'--tmp-path' 否则您将无法检索 "
            errMsg += '命令输出'
            logger.error(errMsg)
        elif isNoneValue(output):
            logger.error('无法检索 xp_cmdshell 输出')
        else:
            logger.info('xp_cmdshell扩展程序可用')
    ...
    def xpCmdshellForgeCmd(self, cmd, insertIntoTable=None):
        # 当用户提供 DBMS 凭据（使用 --dbms-cred）时，我们需要
        # 按顺序将命令标准输出重定向到临时文件
        # 之后检索它
        # 注意：当命令为“del”时不需要执行此操作
        ...
        # 'sqlmapoutput'，除非提供 DBMS 凭据，因为
        # 不幸的是，它不起作用，需要使用 BULK INSERT
        # 使用 OPENROWSET 时检索输出，因此重定向
        # 从上面的临时文件
    ...
    def xpCmdshellInit(self):
        if not kb.xpCmdshellAvailable:
            infoMsg = '检查 xp_cmdshell 扩展过程是否为 '
            infoMsg += '可用，请稍候..'
            logger.info(infoMsg)
            ...
            if result:
                logger.info('xp_cmdshell 扩展程序可用')
                kb.xpCmdshellAvailable = True

            else:
                message = 'xp_cmdshell扩展程序好像没有 '
                message += '可用。你想让 sqlmap 尝试吗 '
                message += '重新启用它？ [是/否] '
                ...
                if readInput(message, default='Y', boolean=True):
                    ...
                    if self._xpCmdshellCheck():
                        ...
                        logger.info('使用 sp_OACreate 创建 xp_cmdshell')
                        ...
                        if self._xpCmdshellCheck():
                            ...
                            warnMsg = 'xp_cmdshell 创建失败，可能 '
                            warnMsg += '因为 sp_OACreate 被禁用'
                            logger.warning(warnMsg)

            hashDBWrite(HASHDB_KEYS.KB_XP_CMDSHELL_AVAILABLE, kb.xpCmdshellAvailable)

            if not kb.xpCmdshellAvailable:
                errMsg = '没有 xp_cmdshell 无法继续'
                raise SqlmapUnsupportedFeatureException(errMsg)

        debugMsg = '创建支持表来编写标准命令 '
        debugMsg += '输出到'
        logger.debug(debugMsg)

        # 此处不能使用 TEXT，因为在错误技术中您会得到：
        # “text、ntext 和 image 数据类型无法进行比较或排序”
