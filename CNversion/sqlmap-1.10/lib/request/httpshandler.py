...
class HTTPSConnection(_http_client.HTTPSConnection):
    ...
    def __init__(self, *args, **kwargs):
        # 注意：https://bugs.python.org/issue38251 / https://github.com/sqlmapproject/sqlmap/issues/4158 的脏补丁
    ...
    def connect(self):
        ...
        # 参考文献：https://docs.python.org/2/library/ssl.html#ssl.SSLContext
            ...
            for protocol in (_ for _ in _protocols if _ >= ssl.PROTOCOL_TLSv1):
                try:
                    ...
                    if protocol not in _contexts:
                        ...
                        # 使用 PROTOCOL_TLS_CLIENT 禁用默认启用的证书和主机名验证
                        ...
                        try:
                            # 参考文献：https://askubuntu.com/a/1263098
                    ...
                    logger.debug('“%s”（“%s”）发生 SSL 连接错误' % (_lut[protocol], getSafeExString(ex)))
        ...
        if not success:
            errMsg = '无法建立 SSL 连接'
            # 参考号：https://docs.python.org/2/library/ssl.html
            if LooseVersion(PYVERSION) < LooseVersion("2.7.9"):
                errMsg += ' （请使用 Python >= 2.7.9 重试）'
