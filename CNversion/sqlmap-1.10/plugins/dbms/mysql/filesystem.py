...
class Filesystem(GenericFilesystem):
    ...
    def linesTerminatedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        ...
        warnMsg = '预计里面有垃圾字符 '
        warnMsg += '文件作为原始查询的剩余部分'
        singleTimeWarnMessage(warnMsg)

        return self.askCheckWrittenFile(localFile, remoteFile, forceCheck)

    def stackedWriteFile(self, localFile, remoteFile, fileType, forceCheck=False):
        debugMsg = '创建一个支持表来写入十六进制 '
        debugMsg += '编码文件到'
        logger.debug(debugMsg)
        ...
        debugMsg = '伪造SQL语句写入十六进制 '
        debugMsg += '编码文件到支持表'
        logger.debug(debugMsg)

        sqlQueries = self.fileToSqlQueries(fcEncodedList)

        logger.debug('将十六进制编码文件插入到支持表中')

        inject.goStacked("SET GLOBAL max_allowed_packet = %d" % (1024 * 1024))  # 1MB（注：https://github.com/sqlmapproject/sqlmap/issues/3230)
        ...
        # 参考号：http://dev.mysql.com/doc/refman/5.1/en/select.html
