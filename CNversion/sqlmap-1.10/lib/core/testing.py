...
def vulnTest():
    ...
    if not vulnserver._alive:
        logger.error("vulnserver实例化时出现问题（地址：'http://%s:%s'）" % (address, port))
        return False
    else:
        logger.info('vulnserver 运行在“http://%s:%s”...' % (address, port))
    ...
    for options, checks in TESTS:
        ...
        dataToStdout('\r[%s] [信息] 完整：%s' % (time.strftime("%X"), status))
