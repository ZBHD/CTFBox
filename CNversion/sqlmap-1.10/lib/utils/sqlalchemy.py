...
try:
    import MySQLdb  # SQLAlchemy 在 MySQL 的情况下使用
...
def getSafeExString(ex, encoding=None):  # 交叉引用功能
...
class SQLAlchemy(GenericConnector):
                ...
                if "_get_server_version_info" in traceback.format_exc():
                    try:
                        ...
                        if int(pymssql.__version__[0]) < 2:
                            raise SqlmapConnectionException("SQLAlchemy 连接问题（过时版本的 pymssql（'%s'）导致问题）" % pymssql.__version__)
                    except ImportError:
                        pass
                elif "invalid literal for int() with base 10: '0b" in traceback.format_exc():
                    raise SqlmapConnectionException("SQLAlchemy 连接问题（'https://bitbucket.org/zzzeek/sqlalchemy/issues/3975'）")
                else:
                    pass
            except SqlmapFilePathException:
                raise
            except Exception as ex:
                raise SqlmapConnectionException("SQLAlchemy 连接问题（'%s'）" % getSafeExString(ex))

            self.printConnected()
        else:
            raise SqlmapMissingDependence('SQLAlchemy 不可用（例如“pip%s 安装 SQLAlchemy”）' % ('3' if six.PY3 else ""))
    ...
    def execute(self, query):
        ...
        # 参考号：https://stackoverflow.com/a/69491015
