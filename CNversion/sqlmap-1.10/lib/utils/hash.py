...
except:  # 由于 https://github.com/sqlmapproject/sqlmap/issues/3171 删除了 ImportError
...
def mysql_old_passwd(password, uppercase=True):  # 版本“4.1”之前
    """
    Reference(s):
...
def mssql_new_passwd(password, salt, uppercase=False):  # 自版本“2012”以来
    """
    Reference(s):
...
def mssql_passwd(password, salt, uppercase=False):  # 版本“2005”和“2008”
    """
    Reference(s):
...
def mssql_old_passwd(password, salt, uppercase=True):  # 版本“2000”及之前
    """
    Reference(s):
...
def oracle_old_passwd(password, username, uppercase=True):  # 版本“11g”之前
    """
    Reference(s):
        http://www.notesbit.com/index.php/scripts-oracle/oracle-11g-new-password-algorithm-is-revealed-by-seclistsorg/
...
def phpass_passwd(password, salt, count, prefix, **kwargs):
    ...
    if prefix == "$S$":
        # 参考号：https://api.drupal.org/api/drupal/includes%21password.inc/constant/DRUPAL_HASH_LENGTH/7.x
        retVal = retVal[:55]
...
def _finalize(retVal, results, processes, attack_info=None):
    ...
    # 注：https://github.com/sqlmapproject/sqlmap/issues/4367
    # 注：https://dzone.com/articles/python-101-creating-multiple-processes
...
def storeHashesToFile(attack_dict):
    ...
    if kb.choices.storeHashes is None:
        message = '你想将哈希值存储到临时文件中吗 '
        message += '用于最终使用其他工具进一步处理[y/N] '
...
def attackDumpedTable():
    if kb.data.dumpedTable:
        ...
        if binary_fields:
            ...
            warnMsg = '检测到潜在的二进制字段（“%s”）。如果您遇到任何问题 ' % _
            warnMsg += '建议使用 \'--fresh-queries --binary-fields="%s"\' 重新运行表转储' % _
        ...
        if attack_dict:
            infoMsg = '识别出 %s 列中可能的密码哈希值 ' % ("s" if len(col_passwords) > 1 else "")
            infoMsg += "'%s'" % ", ".join(col for col in col_passwords)
            logger.info(infoMsg)

            storeHashesToFile(attack_dict)

            message = '你想通过基于字典的攻击来破解它们吗？ %s' % ("[y/N/q]" if conf.multipleTargets else "[Y/n/q]")
...
def hashRecognition(value):
    ...
    if value and len(value) >= 8 and ' ' not in value:   # 注：预过滤条件（用于优化目的）
        ...
        if kb.cache.hashRegex is None:
            ...
            for name, regex in getPublicTypeMembers(HASH):
                # Oracle 和旧 MySQL 的哈希值看起来相同，因此这些检查
        ...
        for word in wordlist:
            ...
            try:
                ...
                if current in hashes:
                    ...
                    status = '当前状态：%s...%s' % (word.ljust(5)[:5], ROTATING_CHARS[rotator])
                ...
                pass  # 忽略自定义词典中某些单词可能引起的编码问题

            except Exception as ex:
                warnMsg = "哈希条目时出现问题：%s（'%s'）。 " % (repr(word), getSafeExString(ex))
                warnMsg += '请通过电子邮件举报至“%s”' % DEV_EMAIL_ADDRESS
    ...
    try:
        for word in wordlist:
            ...
            try:
                ...
                if hash_ == current:
                    if hash_regex == HASH.ORACLE_OLD:  # 仅用于美容目的
                        word = word.upper()
...
def dictionaryAttack(attack_dict):
    ...
    if conf.disableMulti:
        ...
        try:
            ...
            # FreeBSD 上的问题（参考：https://web.archive.org/web/20110710041353/http://www.eggheadcafe.com/microsoft/Python/35880259/multiprocessing-on-freebsd.aspx)
            _ = multiprocessing.Queue()

            # ctypes的问题（参考：https://github.com/sqlmapproject/sqlmap/issues/2952)
                ...
                if re.match(hash_regex, hash_):
                    try:
                        ...
                        if hash_regex in (HASH.MD5_BASE64, HASH.SHA1_BASE64, HASH.SHA256_BASE64, HASH.SHA512_BASE64):
                            ...
                            if hash_.startswith("0x"):  # 参考号：https://docs.microsoft.com/en-us/sql/t-sql/functions/hashbytes-transact-sql?view=sql-server-2017
                                hash_ = hash_[2:]
                            item = [(user, hash_), {}]
                        ...
                        if item and hash_ not in keys:
                            ...
                            if not resumed:
                                ...
                                infoMsg = '恢复哈希“%s”的密码“%s”' % (resumed, hash_)
                                if user and not user.startswith(DUMMY_USER_PREFIX):
                                    infoMsg += ' 对于用户“%s”' % user
                                logger.info(infoMsg)
                ...
                try:
                    if _multiprocessing:
                        if _multiprocessing.cpu_count() > 1:
                            infoMsg = '启动 %d 进程 ' % _multiprocessing.cpu_count()
                        ...
                        warnMsg = '多处理哈希破解目前 '
                        warnMsg += '本平台上的%s' % ("not supported" if not conf.disableMulti else "disabled")
                    ...
                    warnMsg = '用户在基于字典的攻击阶段中止（按下 Ctrl+C）'
                    logger.warning(warnMsg)
