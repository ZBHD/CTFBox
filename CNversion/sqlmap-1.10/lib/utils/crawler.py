...
def crawl(target, post=None, cookie=None):
    ...
    try:
        ...
        def crawlThread():
            ...
            while kb.threadContinue:
                ...
                try:
                    ...
                    errMsg = '检测到连接异常（“%s”）。跳绳 ' % getSafeExString(ex)
                    errMsg += "URL '%s'" % current
                    logger.critical(errMsg)
                except SqlmapSyntaxException:
                    errMsg = '检测到无效 URL。跳过“%s”' % current
                    logger.critical(errMsg)
                except _http_client.InvalidURL as ex:
                    errMsg = '检测到无效 URL（“%s”）。跳绳 ' % getSafeExString(ex)
                    errMsg += "URL '%s'" % current
                    logger.critical(errMsg)
                ...
                if conf.verbose in (1, 2):
                    ...
                    status = '%d/%d 链接已访问 (%d%%)' % (threadData.shared.count, threadData.shared.length, round(100.0 * threadData.shared.count / threadData.shared.length))
        ...
        if kb.checkSitemap:
            ...
            try:
                ...
                if "page not found" in getSafeExString(ex):
                    found = False
                    logger.warning('找不到“sitemap.xml”')
            except:
                pass
            finally:
                if found:
        ...
        if not conf.bulkFile:
            infoMsg = '正在启动目标 URL“%s”的爬取程序' % target
            logger.info(infoMsg)
