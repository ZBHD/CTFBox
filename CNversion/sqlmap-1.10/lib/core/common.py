...
class Format(object):
    ...
    def getDbms(versions=None):
        ...
        # 注意：防止丑陋的（例如）“后端数据库管理系统：MySQL 未知”
...
class Backend(object):
    ...
    def setDbms(dbms):
        ...
        # 没有什么预防措施，理论上这个条件应该总是错误的
        elif kb.dbms is not None and kb.dbms != dbms:
            warnMsg = '似乎很有可能 '
            warnMsg += '这可能是误报案例'
            logger.warning(warnMsg)

            msg = 'sqlmap 之前将后端数据库管理系统 指纹识别为 '
            msg += '%s。不过现在已经被指纹识别了 ' % kb.dbms
            msg += "as %s. " % dbms
            msg += '请指定哪个 DBMS 应该是 '
            msg += '正确[%s（默认）/%s] ' % (kb.dbms, dbms)
    ...
    def setVersionList(versionsList):
        if isinstance(versionsList, list):
            ...
            logger.error('versionsList 的格式无效')
    ...
    def setOs(os):
        ...
        # 没有什么预防措施，理论上这个条件应该总是错误的
            ...
            msg = 'sqlmap 先前对后端数据库管理系统 进行了指纹识别 '
            msg += '操作系统 %s。然而现在它已经 ' % kb.os
            msg += '指纹识别为%s。 ' % os
            msg += '请指定哪个操作系统 '
            msg += '正确[%s（默认）/%s] ' % (kb.os, os)
    ...
    def isDbms(dbms):
        if not kb.get("testMode") and all((Backend.getDbms(), Backend.getIdentifiedDbms())) and Backend.getDbms() != Backend.getIdentifiedDbms():
            singleTimeWarnMessage('已识别（“%s”）和指纹（“%s”）DBMS 不同。如果您在枚举阶段遇到问题，请使用“--flush-session”重新运行' % (Backend.getIdentifiedDbms(), Backend.getDbms()))
            ...
            if condition:
                ...
                if not conf.multipleTargets and not (conf.csrfToken and re.search(conf.csrfToken, parameter, re.I)):
                    ...
                    if (_.endswith("'") and _.count("'") == 1 or re.search(r'\A9{3,}', _) or re.search(r'\A-\d+\Z', _) or re.search(DUMMY_USER_INJECTION, _)) and not re.search(GOOGLE_ANALYTICS_COOKIE_REGEX, parameter):
                        warnMsg = '您似乎提供了受污染的参数值 '
                        warnMsg += "('%s') 最有可能是剩余的 " % element
                        warnMsg += '来自手动 SQL 注入测试的字符/语句。 '
                        warnMsg += '请始终仅使用有效的参数值 '
                        warnMsg += '这样sqlmap就可以正常运行了'
                        logger.warning(warnMsg)

                        message = '您真的确定要继续吗（sqlmap 可能有问题）？ [是/否] '
                    ...
                    for regex in (r"\A((?:<[^>]+>)+\w+)((?:<[^>]+>)+)\Z", r"\A([^\w]+.*\w+)([^\w]+)\Z"):
                        ...
                        if match:
                            try:
                                ...
                                # 注意：对于内部有自定义注入标记的情况（例如 https://github.com/sqlmapproject/sqlmap/issues/4137#issuecomment-2013783111) - p.s. 不太关心结构（例如注入平面数组值）
                                ...
                                if candidates:
                                    ...
                                    message += 'JSON 是可反序列化的。你想注射到里面吗？ [是/否] '
                            ...
                            message = '似乎为 %s 参数“%s”提供了值 ' % ("%s " % place if place != parameter else "", parameter)
                            message += "有界限。你想注射到里面吗？ ('%s') [是/否] " % getUnicode(_)
...
def getManualDirectories():
    ...
    if conf.webRoot:
        ...
        infoMsg = '使用“%s”作为 Web 服务器文档根' % conf.webRoot
        logger.info(infoMsg)
    elif directories:
        infoMsg = '检索到 Web 服务器文档根：“%s”' % directories
        logger.info(infoMsg)
    else:
        warnMsg = '无法自动检索网络服务器 '
        warnMsg += '文档根目录'
...
def setColor(message, color=None, bold=False, level=None, istty=None):
    ...
    if message:
        if (IS_TTY or istty) and not conf.get("disableColoring"):  # 着色处理程序
            if level is None:
...
def dataToTrafficFile(data):
    ...
    try:
        ...
        errMsg = '尝试时出了点问题 '
        errMsg += '写入流量文件“%s”（“%s”）' % (conf.trafficFile, getSafeExString(ex))
...
def dataToDumpFile(dumpFile, data):
    try:
        ...
        if "No space left" in getUnicode(ex):
            ...
            errMsg = "将转储数据写入文件时发生错误（'%s'）" % getUnicode(ex)
            logger.error(errMsg)
...
def dataToOutFile(filename, data):
    ...
    if data:
        while True:
            ...
            try:
                with open(retVal, "w+b") as f:  # 必须保留为非编解码器，因为数据是原始 ASCII 编码数据
...
def setPaths(rootPath):
    ...
    if IS_WIN:
        # 参考号：https://pureinfotech.com/list-environment-variables-windows-10/
        ...
        if dbmsName == conf.dbms or conf.dbms.lower() in data[0]:
            try:
                ...
                if dbmsName in (DBMS.ACCESS, DBMS.SQLITE, DBMS.FIREBIRD):
                    if remote:
                        warnMsg = '通过网络直接连接 '
                        warnMsg += '%s 不支持 DBMS' % dbmsName
                        logger.warning(warnMsg)

                        conf.hostname = "localhost"
                        conf.port = 0
                elif not remote:
                    errMsg = '缺少远程连接详细信息（例如 '
                    errMsg += "'mysql://USER:PASSWORD@DBMS_IP:DBMS_PORT/DATABASE_NAME' "
                    errMsg += '或“访问://DATABASE_FILEPATH”）'
                    raise SqlmapSyntaxException(errMsg)
                ...
                if _sqlalchemy and data[3] and any(_ in _sqlalchemy.dialects.__all__ for _ in (data[3], data[3].split('+')[0])):
                    ...
                    errMsg += '为了直接连接到DBMS '
                    errMsg += '“%s”。您可以从“%s”下载' % (dbmsName, data[2])
                    errMsg += '。另一种方法是使用包“python-sqlalchemy” '
                    errMsg += '安装了对方言“%s”的支持' % data[3]
                    raise SqlmapMissingDependence(errMsg)
...
def parseTargetUrl():
    ...
    try:
        ...
        errMsg = '提供了无效的 URL“%s”（“%s”）。 ' % (conf.url, getSafeExString(ex))
        errMsg += '请确保您没有任何剩余字符（例如“[”或“]”） '
        errMsg += '在主机名部分'
        raise SqlmapGenericException(errMsg)
    ...
    if any((invalid, re.search(r"\s", conf.hostname), '..' in conf.hostname, conf.hostname.startswith('.'), '\n' in originalUrl)):
        errMsg = "目标 URL 无效（'%s'）" % originalUrl
        raise SqlmapSyntaxException(errMsg)
    ...
    if conf.port < 1 or conf.port > 65535:
        errMsg = '目标 URL 端口无效 (%d)' % conf.port
        raise SqlmapSyntaxException(errMsg)
    ...
    if (intersect(REFERER_ALIASES, conf.testParameter, True) or conf.level >= 3) and not any(_[0].upper() == HTTP_HEADER.REFERER.upper() for _ in conf.httpHeaders):
        debugMsg = '将 HTTP Referer 请求头设置为目标 URL'
        logger.debug(debugMsg)
    ...
    if (intersect(HOST_ALIASES, conf.testParameter, True) or conf.level >= 5) and not any(_[0].upper() == HTTP_HEADER.HOST.upper() for _ in conf.httpHeaders):
        debugMsg = '将 HTTP Host 请求头设置为目标 URL'
        logger.debug(debugMsg)
    ...
    if isinstance(params, six.string_types):
        ...
        if retVal.count("%s", start, end) == len(params):
            ...
            if count > len(params) and count % len(params):
                ...
                warnMsg += '请通过邮件内容“%r | %r | %r”举报至“%s”' % (format_, params, retVal, DEV_EMAIL_ADDRESS)
...
def parseXmlFile(xmlFile, handler):
    ...
    try:
        ...
        errMsg = '似乎有问题 '
        errMsg += '文件“%s”（“%s”）。请使 ' % (xmlFile, getSafeExString(ex))
...
def getSQLSnippet(dbms, sfile, **variables):
    ...
    if variables:
        errMsg = 'SQL 文件“%s”中未解析的变量%s“%s”' % ("s" if len(variables) > 1 else "", ", ".join(variables), sfile)
        logger.error(errMsg)
        ...
        with kb.locks.cache:
            if filename not in kb.cache.content:
                ...
                try:
                    ...
                    errMsg = '尝试时出了点问题 '
                    errMsg += '读取文件“%s”的内容（“%s”）' % (filename, getSafeExString(ex))
                    raise SqlmapSystemException(errMsg)
...
def goGoodSamaritan(prevValue, originalCharset):
    ...
    if kb.partRun in kb.commonOutputs:
        ...
        for item in commonPartOutputs:
            # 检查公共输出（item）是否以 prevValue 开头
            # 其中 prevValue 是迄今为止枚举的字符
...
def getPartRun(alias=True):
    ...
    # 参考号：http://coding.derkeiler.com/Archive/Python/comp.lang.python/2004-06/2267.html
    except TypeError:
        pass

    # 返回 INI 标记以考虑常见输出（例如“数据库”）
    if alias:
...
def wasLastResponseDelayed():
    ...
    # 99.9999999997440% of 所有非基于时间的 SQL 注入均受影响
    # 响应时间应在 +-7*stdev([正常响应时间]) 之内
    # 数学参考：http://www.answers.com/topic/standard-deviation
    ...
    if deviation and not conf.direct and not conf.disableStats:
        if len(kb.responseTimes[kb.responseTimeMode]) < MIN_TIME_RESPONSES:
            warnMsg = '模型中使用的基于时间的标准偏差方法 '
            warnMsg += '响应时间小于 %d' % MIN_TIME_RESPONSES
            logger.warning(warnMsg)
        ...
        if not kb.testMode and retVal:
            if kb.adjustTimeDelay is None:
                msg = '你想让 sqlmap 尝试优化值吗 '
                msg += '对于 DBMS 延迟响应（选项“--time-sec”）？ [是/否] '
        ...
        if Backend.getIdentifiedDbms() in (DBMS.MYSQL,):  # MySQL 的 SLEEP(X) 持续时间平均缩短 0.05 秒
            delta += 0.05
        return delta >= 0
...
def adjustTimeDelay(lastQueryDuration, lowerStdLimit):
    ...
    if all((_ == candidate for _ in kb.delayCandidates)) and candidate < conf.timeSec:
        if lastQueryDuration / (1.0 * conf.timeSec / candidate) > MIN_VALID_DELAYED_RESPONSE:  # 注意：为了防止像 RANDOMBLOB 这样的大量查询的快速响应问题
            conf.timeSec = candidate

            infoMsg = '将时间延迟调整为 '
            infoMsg += '%d 由于良好的响应时间而排名第二 %s' % (conf.timeSec, 's' if conf.timeSec > 1 else '')
            logger.info(infoMsg)
...
def urldecode(value, encoding=None, unsafe="%%?&=;+%s" % CUSTOM_INJECTION_MARK_CHAR, convall=False, spaceplus=True):
    ...
    if value:
        ...
        if convall:
            ...
            if spaceplus:
                result = result.replace('+', ' ')  # 加号在 URL 编码数据中具有特殊含义（因此在 conval 情况下使用 _urllib.parse.unquote_plus）
...
def urlencode(value, safe="%&=-_", convall=False, limit=False, spaceplus=False):
    ...
    if value:
        ...
        if Backend.isDbms(DBMS.MSSQL) and not kb.tamperFunctions and any(ord(_) > 255 for _ in value):
            warnMsg = '如果您遇到问题 '
            warnMsg += '非 ASCII 标识符名称 '
            warnMsg += '建议您使用“--tamper=charunicodeencode”重新运行'
...
def runningAsAdmin():
    ...
    if PLATFORM in ("posix", "mac"):
        ...
        errMsg = 'sqlmap 无法检查您是否正在运行它 '
        errMsg += '作为该平台的管理员帐户。 '
        errMsg += 'sqlmap 会假设您是管理员 '
        errMsg += '这对于所请求的接管攻击是强制性的 '
        errMsg += '正常工作'
        logger.error(errMsg)
...
def getPageTemplate(payload, place):  # 交叉引用功能
...
def showHttpErrorCodes():
    ...
    if kb.httpErrorCodes:
        ...
        if any((str(_).startswith('4') or str(_).startswith('5')) and _ != _http_client.INTERNAL_SERVER_ERROR and _ != kb.originalCode for _ in kb.httpErrorCodes):
            msg = '太多 4xx 和/或 5xx HTTP 错误代码 '
            msg += '可能意味着涉及某种保护（例如 WAF）'
            logger.debug(msg)

def openFile(filename, mode='r', encoding=UNICODE_ENCODING, errors="reversible", buffering=1):  # “buffering=1”表示行缓冲（参考：http://stackoverflow.com/a/3168436)
    """
    ...
    # 参考号：https://stackoverflow.com/a/37462452
    if 'b' in mode:
        buffering = 0
        ...
        try:
            if value > 255:
                ...
                if Backend.isDbms(DBMS.MYSQL):
                    # 参考号：https://dev.mysql.com/doc/refman/8.0/en/string-functions.html#function_ord
                    # 注：https://github.com/sqlmapproject/sqlmap/issues/1531
                    retVal = getUnicode(raw, conf.encoding or UNICODE_ENCODING)
                elif Backend.isDbms(DBMS.MSSQL):
                    # 参考号：https://docs.microsoft.com/en-us/sql/relational-databases/collations/collation-and-unicode-support?view=sql-server-2017 和 https://stackoverflow.com/a/14488478
                    retVal = getUnicode(raw, "UTF-16-BE")
                elif Backend.getIdentifiedDbms() in (DBMS.PGSQL, DBMS.ORACLE, DBMS.SQLITE):     # 注意：带有 Unicode 代码点的情况（例如 http://www.postgresqltutorial.com/postgresql-ascii/)
...
def unhandledExceptionMessage():
    ...
    errMsg += '使用官方 GitHub 上的最新开发版本运行 '
    errMsg += '存储库位于“%s”。如果异常仍然存在，请打开一个新问题 ' % GIT_PAGE
    errMsg += "at '%s' " % ISSUES_PAGE
    errMsg += '包含以下文本以及所需的任何其他信息 '
    errMsg += '重现该错误。开发人员将尝试重现该错误，并进行相应修复 '
    errMsg += '并回复你\n'
...
def fetchRandomAgent():
    ...
    if not kb.userAgents:
        debugMsg = '从以下位置加载随机 HTTP User-Agent 请求头 '
        ...
        try:
            ...
            errMsg = '无法读取 HTTP User-Agent 请求头 '
    ...
    if choice:
        ...
        try:
            ...
            if duplicate:
                ...
                if closed:
                    warnMsg += ' 并解决了。请更新到最新 '
                    warnMsg += '开发版本来自官方 GitHub 存储库“%s”' % GIT_PAGE
                logger.warning(warnMsg)
                return
        except:
            pass
...
def removeReflectiveValues(content, payload, suppressWarning=False):
    ...
    try:
        if all((content, payload)) and isinstance(content, six.text_type) and kb.reflectiveMechanism and not kb.heuristicMode:
            ...
            # 注意：特殊情况下，结果的一部分与载荷共享相同的输出（例如？id=1...和“sqlmap/1.0-dev (http://sqlmap.org)”)
            ...
            if regex != payload:
                if all(part.lower() in content.lower() for part in filterNone(regex.split(REFLECTED_REPLACEMENT_REGEX))[1:]):  # 快速优化检查
                    ...
                    if len(parts) > REFLECTED_MAX_REGEX_PARTS:  # 防止CPU占用
                ...
                if retVal != content:
                    ...
                    if re.search(r"(?i)FRAME[^>]+src=[^>]*%s" % REFLECTED_VALUE_MARKER, retVal):
                        warnMsg = '检测到包含受攻击参数值的帧。请务必 '
                        warnMsg += '单独测试这些，以防对此页面的攻击失败'
                        singleTimeWarnMessage(warnMsg)
...
def safeSQLIdentificatorNaming(name, isTable=False):
    ...
    if isinstance(name, six.string_types):
        ...
        # 注意：SQL 92 对以下划线开头的标识符有限制（例如 http://www.frontbase.com/documentation/FBUsers_4.pdf)
        if retVal.upper() in kb.keywords or (not isTable and (retVal or " ")[0] == '_') or (retVal or " ")[0].isdigit() or not re.match(r"\A[A-Za-z0-9_@%s\$]+\Z" % ('.' if _ else ""), retVal):  # MsSQL 是唯一一个我们自动在表名前添加模式的 DBMS（点是正常的）
            if not conf.noEscape:
                retVal = unsafeSQLIdentificatorNaming(retVal)

                if Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.ACCESS, DBMS.CUBRID, DBMS.SQLITE):  # 注意：在 SQLite 中，如果列/标识符不存在，双引号将被视为字符串（例如 SELECT "foobar" FROM users）
                    retVal = "`%s`" % retVal
        ...
        if _ and DEFAULT_MSSQL_SCHEMA not in retVal and '.' not in re.sub(r"\[[^]]+\]", "", retVal):
            if (conf.db or "").lower() != "information_schema":     # 注：https://github.com/sqlmapproject/sqlmap/issues/5192
    ...
    for mnemonic in (mnemonics or "").split(','):
        ...
        if found:
            ...
            if value is not None:
                ...
                errMsg = '助记符“%s”需要类型“%s”的值' % (name, found.type)
                raise SqlmapSyntaxException(errMsg)
...
def asciifyUrl(url, forceQuote=False):
    ...
    # UTF8-引用其他部分。我们单独检查每个部分，如果
    # 如果需要引用 - 这应该会吸引一些额外的用户
    # 错误，例如用户名中的变音符号，即使
    # 路径 *is* 已被引用。
    def quote(s, safe):
        s = s or ''
        # 非 ascii 字符上的触发器 - 另一种选择是：
...
def findPageForms(content, url, raiseException=False, addToTargets=False):
    ...
    try:
        ...
        if re.search(r"(?i)<!DOCTYPE html|<html", content or "") and not re.search(r"(?i)\.js(\?|\Z)", url):
            dbgMsg = "给定 URL ('%s') 处的 HTML 格式错误。打算过滤一下" % url
            logger.debug(dbgMsg)
    ...
    for form in forms or []:
        try:
            ...
            errMsg = '出现问题 '
            errMsg += "处理页面表单（'%s'）" % getSafeExString(ex)
            if raiseException:
                raise SqlmapGenericException(errMsg)
            else:
                logger.debug(errMsg)
...
def checkSystemEncoding():
    ...
    if sys.getdefaultencoding() == "cp720":
        try:
            ...
            errMsg = '有一个已知的 Python 问题 (#1616979) 相关 '
...
def evaluateCode(code, variables=None):
    ...
    try:
        ...
        errMsg = "评估提供的代码时发生错误（'%s'） " % getSafeExString(ex)
        raise SqlmapGenericException(errMsg)
    ...
    def _(value):
        ...
        if value and isinstance(value, six.string_types):
            ...
            if len(value) % 2 != 0:
                ...
                singleTimeWarnMessage('从预期的十六进制形式解码值“%s”时出现问题' % value)
            else:
    ...
    if not conf.loadCookies:
        ...
        try:
            if not cookieJar.filename:
                infoMsg = '从“%s”加载cookie' % conf.loadCookies
                logger.info(infoMsg)
                ...
                # 参考号：http://www.hashbangcode.com/blog/netscape-http-cooke-file-parser-php-584.html
            ...
            if not cookieJar._cookies:
                errMsg = '未找到有效的 cookie'
                raise SqlmapGenericException(errMsg)

        except Exception as ex:
            errMsg = '加载时出现问题 '
            errMsg += "cookie 文件（'%s'）" % re.sub(r"(cookies) file '[^']+'", r"\g<1>", getSafeExString(ex))
            raise SqlmapGenericException(errMsg)
...
def parseRequestFile(reqFile, checkParams=True):
    ...
    def _parseWebScarabLog(content):
        ...
        for request in reqResList:
            ...
            if method.upper() == HTTPMETHOD.POST:
                warnMsg = '不支持来自 WebScarab 日志的 POST 请求 '
                warnMsg += '因为它们的正文内容存储在单独的文件中。 '
                warnMsg += '不过，您可以使用 -r 单独加载它们。'
                logger.warning(warnMsg)
                continue
    ...
    if conf.scope:
        logger.info("使用正则表达式'%s'来过滤目标" % conf.scope)
