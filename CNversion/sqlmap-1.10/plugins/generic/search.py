...
class Search(object):
    ...
    def searchTable(self):
        ...
        if Backend.isDbms(DBMS.MYSQL) and not kb.data.has_information_schema:
            errMsg = 'information_schema 不可用， '
            errMsg += '后端数据库管理系统 是 MySQL < 5.0'
            bruteForce = True
        ...
        for tbl in tblList:
            ...
            if dbCond and conf.db:
                ...
                infoMsg += " 对于数据库%s '%s'" % ("s" if len(_) > 1 else "", ", ".join(db for db in _))
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                for db in foundTbls:
                    ...
                    if tblConsider == "1":
                        infoMsg += "s LIKE"
                    infoMsg += ' 数据库“%s”中的“%s”' % (unsafeSQLIdentificatorNaming(tbl), unsafeSQLIdentificatorNaming(db))
                    logger.info(infoMsg)
                    ...
                    if not isNumPosStrValue(count):
                        ...
                        if tblConsider == "1":
                            warnMsg += "s LIKE"
                        warnMsg += " '%s' " % unsafeSQLIdentificatorNaming(tbl)
                        warnMsg += '在数据库“%s”中' % unsafeSQLIdentificatorNaming(db)
                        logger.warning(warnMsg)
            ...
            if not values and isInferenceAvailable() and not conf.direct:
                ...
                for column, dbData in foundCols.items():
                    ...
                    for db in dbData:
                        ...
                        if colConsider == "1":
                            infoMsg += "s LIKE"
                        infoMsg += ' 数据库“%s”中的“%s”' % (unsafeSQLIdentificatorNaming(column), unsafeSQLIdentificatorNaming(db))
                        logger.info(infoMsg)
