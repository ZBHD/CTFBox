...
class Agent(object):
    ...
    def payloadDirect(self, query):
        ...
        if Backend.getIdentifiedDbms() in (DBMS.ORACLE,):  # 非标准对象在返回时会给数据库连接器带来问题（例如 XMLTYPE）
    ...
    def payload(self, place=None, parameter=None, value=None, newValue=None, where=None):
        ...
        if base64Encoding:
            ...
            # 注：https://github.com/sqlmapproject/sqlmap/issues/5488
    ...
    def prefixQuery(self, expression, prefix=None, where=None, clause=None):
        ...
        # 如果我们将 (<where>) 参数原始值替换为
        # 我们的载荷不添加前缀
        if where == PAYLOAD.WHERE.REPLACE and not conf.prefix:  # 注：https://github.com/sqlmapproject/sqlmap/issues/4030
            query = ""

        # 如果该技术是堆叠查询（<stype>），则不要放置空格
        # 在前缀之后或者在 GROUP BY / ORDER BY (<clause>) 中
        ...
        # 在任何其他情况下，请在前面添加完整的前缀
        else:
            query = kb.injection.prefix or prefix or ""

            if "SELECT '[RANDSTR]'" in query:  # 转义 WHERE 之前的前缀
    ...
    def adjustLateValues(self, payload):
        ...
        if payload:
            ...
            # 注：https://github.com/sqlmapproject/sqlmap/issues/5057
    ...
    def hexConvertField(self, field):
        ...
        if "hex" in rootQuery and hasattr(rootQuery.hex, "query"):
            ...
            warnMsg = 'DBMS“%s”当前不支持开关“--hex”' % Backend.getIdentifiedDbms()
    ...
    def nullCastConcatFields(self, fields):
        ...
        if fields.startswith("(CASE") or fields.startswith("(IIF") or fields.startswith("SUBSTR") or fields.startswith("MID(") or re.search(r"\A'[^']+'\Z", fields):
            ...
            for field in fieldsSplitted:
                field = re.sub(r"(?i) AS \w+\Z", "", field)         # 注意：必须从别名部分中删除诸如“... AS type_name”之类的字段，此功能才能发挥作用
    ...
    def limitCondition(self, expression, dump=False):
        ...
        # 我们假设只有不包含“LIMIT #, 1”的查询
        # （或等效项，具体取决于后端数据库管理系统）可以返回
        # 多个条目
        if limitCond:
            if (limitRegExp or limitRegExp2) and stopLimit is not None:
                stopLimit = int(stopLimit)

                # 从现在开始我们只需要表达式直到“LIMIT”
                # （或同等内容，取决于后端数据库管理系统）字
        ...
        if Backend.getIdentifiedDbms() in (DBMS.MYSQL, DBMS.PGSQL, DBMS.SQLITE, DBMS.VERTICA, DBMS.PRESTO, DBMS.MIMERSQL, DBMS.CUBRID, DBMS.EXTREMEDB, DBMS.DERBY):
            ...
            if forgeNotIn:
                ...
                if " ORDER BY " not in fromFrom:
                    # 参考号：https://web.archive.org/web/20150218053955/http://vorg.ca/626-the-MS-SQL-equivalent-to-MySQLs-limit-command
