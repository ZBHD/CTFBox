...
def singleTimeWarnMessage(message):  # 交叉引用功能
    sys.stdout.write(message)
    sys.stdout.write("\n")
    sys.stdout.flush()

def filterNone(values):  # 交叉引用功能
    return [_ for _ in values if _] if isinstance(values, _collections.Iterable) else values

def isListLike(value):  # 交叉引用功能
    return isinstance(value, (list, tuple, set, BigArray))

def shellExec(cmd):  # 交叉引用功能
...
def decodeBase64(value, binary=True, encoding=None):
    ...
    # 参考号：https://stackoverflow.com/a/49459036
    if not value.endswith(padding):
        value += 3 * padding

    # 参考号：https://en.wikipedia.org/wiki/Base64#URL_applications
    # 参考号：https://perldoc.perl.org/MIME/Base64.html
...
def getUnicode(value, encoding=None, noneToNull=False):
    ...
    if isinstance(value, six.text_type):
        ...
        try:
            ...
            return six.text_type(str(value), errors="ignore")  # 非基本字符串实例的编码被忽略
